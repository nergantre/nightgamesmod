...
public class ImpMale extends Pet {
	...
	public void act(Combat c, Character target) {
		if(target.human()){
			...
			case 3:
				c.write(owner(),"Your imp masturbates frantically until he cums intensely. He aims his spurting cock at "+target.name()+", hitting her in the face with a thick load " +
	...
	public void vanquish(Combat c, Pet opponent) {
		...
		case fairyfem:
			c.write(owner(),own()+"imp manages to catch "+opponent.own()+"faerie as the tiny fae flies around his head. He shoves his cock into the faerie's face, smearing his pre-cum over her. " +
		...
		case fairymale:
			...
			break;
		case impfem:
			c.write(owner(),own()+"imp grabs "+opponent.own()+"female imp and bends her over. He rams his cock into her wet box without any foreplay. The female groans in protest and flails about " +
		...
		case impmale:
			...
			break;
		case slime:
			c.write(owner(),own()+"imp pins "+opponent.own()+"slime under foot and lets his leaking cock drip onto the amorphous mass. As the slime absorbs the first drops of pre-cum, it starts to " +
	...
	public void caught(Combat c, Character captor) {
		if(owner().human()){
			c.write(captor,captor.name()+" shoves your imp to the floor and pins its cock under her foot. She grinds her foot, lubricated with the pre-cum that's streaming from " +
