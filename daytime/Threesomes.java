...
public class Threesomes implements Scene {
	...
	public void visit(String choice) {
		...
		if(choice=="CassieJewel"){
			Global.gui().message("You step out of the shower room with a towel wrapped around your waist to preserve your modesty. One of these days you should get a proper bathrobe, " +
					...
					"what this accomplishes. It's not like "+player.name()+" was avoiding us. We could have just knocked on the door. Also you should be careful with what you're holding. " +
					...
					"of "+player.name()+"'s parts, aren't I?\"</i> She's enjoying herself so much she's practically purring in your ear. <p>" +
					...
					"need to make it up to her, but Cassie was right, this could have been resolved by just talking to you. <i>\"The reason I captured "+player.name()+" like this is because " +
					...
					"Cassie shakes her head. <i>\"I just said I wouldn't get jealous. I want to thank you for your help. "+player.name()+" is very good with his tongue, I can personally vouch " +
		...
		if(choice=="MaraJewel"){
			Global.gui().message("You're sitting in your room, working on some homework, when you hear a knock on your door. You open the door to find Mara, who pushes past you into " +
					...
					"Jewel leans down, pressing her breasts against your face before sitting up. <i>\"Most bullies don't deserve as much affection as "+player.name()+". I just wish I was a " +
					...
					"Jewel ignores your protest and giggles. <i>\"It's tempting, but I don't want to share "+player.name()+"'s money shot with a bunch of strangers. I think we should be greedy and " +
		...
		if(choice=="AngelMara"){
			Global.gui().message("'Cum over ASAP.'<p>" +
	...
	public String getScene(){
		if(Global.random(5)==0){
			...
			available.add("");
			if(Global.getNPC("Cassie").getAffection(player)>=20&&Global.getNPC("Jewel").getAffection(player)>=20&&Global.getNPC("Jewel").getAffection(Global.getNPC("Cassie"))>=5&&
					Global.getValue(Flag.CassieLoneliness)>=5){
				available.add("CassieJewel");
			}
			if(Global.getNPC("Mara").getAffection(player)>=20&&Global.getNPC("Jewel").getAffection(player)>=20&&Global.getNPC("Jewel").getAffection(Global.getNPC("Mara"))>=5){
				available.add("MaraJewel");
			}
			if(Global.getNPC("Mara").getAffection(player)>=15&&Global.getNPC("Angel").getAffection(player)>=15&&Global.getNPC("Angel").getAffection(Global.getNPC("Mara"))>=10){
