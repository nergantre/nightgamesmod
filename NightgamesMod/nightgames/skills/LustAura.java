...
public class LustAura extends Skill {
	...
	public boolean resolve(Combat c, Character target) {
		...
		target.add(c,
				new Horny(target, (float) (3 + 2 * getSelf().getExposure()),
						3 + Global.random(3),
						getSelf().nameOrPossessivePronoun() + " aura of lust"));
	...
	public String deal(Combat c, int damage, Result modifier,
			Character target) {
		return "You allow the corruption in your libido to spread out from your body. "
				+ target.name()
	...
	public String receive(Combat c, int damage, Result modifier,
			Character target) {
		return getSelf().name()
				+ " releases an aura of pure sex. You feel your body becoming hot just being near her.";
	}
