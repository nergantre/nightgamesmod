...
public class Squeeze extends Skill {
	...
	public boolean usable(Combat c, Character target) {
		return target.hasBalls() && c.getStance().reachBottom(getSelf())
		...
		if (target.roll(this, c, accuracy(c))) {
			if (target.has(Trait.brassballs)) {
				...
				if (getSelf().has(Item.ShockGlove)
					...
					target.pain(c,
							Global.random(8 + target.get(Attribute.Perception))
									+ 16);
	...
	public String deal(Combat c, int damage, Result modifier,
		...
		if (modifier == Result.miss) {
			return "You try to grab " + target.name()
					+ "'s balls, but she avoids it.";
		} else if (modifier == Result.special) {
			return "You use your shock glove to deliver a painful jolt directly into "
					+ target.name() + "'s testicles.";
		} else if (modifier == Result.weak) {
			return "You grab the bulge in " + target.name()
					+ "'s " + target.getOutfit()
							.getTopOfSlot(ClothingSlot.bottom).getName()
					+ " and squeeze.";
		} else if (modifier == Result.weak2) {
			return "You grab " + target.name()
					+ "by the balls and squeeze hard, but" + target.pronoun()
	...
	public String receive(Combat c, int damage, Result modifier,
		...
		if (modifier == Result.miss) {
			...
			return getSelf().name()
					+ " grabs your balls through your " + target.getOutfit()
							.getTopOfSlot(ClothingSlot.bottom).getName()
					+ " and squeezes hard.";
		} else if (modifier == Result.weak2) {
			return getSelf().name()
					+ " grins menacingly and firmly grabs your nuts. "
					+ Global.capitalizeFirstLetter(getSelf().pronoun())
					+ " squeezes as hard as " + getSelf().pronoun()
					+ "can, but you hardly feel it.";
		} else if (modifier == Result.item) {
			return getSelf().name()
					+ " grabs your crotch through your " + target.getOutfit()
							.getTopOfSlot(ClothingSlot.bottom).getName()
