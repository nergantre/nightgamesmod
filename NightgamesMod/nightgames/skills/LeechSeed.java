...
public class LeechSeed extends Skill {
	...
	public boolean usable(Combat c, Character target) {
		return getSelf().canRespond()&&getSelf().body.has("tentacles")&&!target.is(Stsflag.seeded)&&!(target.is(Stsflag.pegged)&&target.is(Stsflag.fucked));
	}
	...
	public String receive(Combat c, int damage, Result modifier, Character target) {
		...
		if(modifier == Result.miss){
			return getSelf().subject() + " tries to plant a seed in you with " + getSelf().possessivePronoun() + " tentacle, but you dodges out of the way";
		}
		String hole = "pussy";
		if (modifier == Result.anal) {
			hole = "ass";
		}
		return Global.format("{self:SUBJECT} flashes a brilliant smile at you and beckons you forward. Against your better judgement, you move closer to {self:direct-object}, hoping for an opening to attack. "
				...
				+ "{self:pronoun} planted a fist sized seed inside you!", getSelf(), target, hole, hole);
	}
