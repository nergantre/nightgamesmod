		...
		if (c.getStance().reachTop(getSelf()) && !target.getOutfit().slotEmpty(ClothingSlot.top)) {
			...
			if (!article.is(ClothingTrait.indestructible)
				...
				if (getSelf().human()) {
					...
					c.write(getSelf(),
							getSelf().name() + " lunges towards you and rakes her nails across your " + article
									.getName() + ", shredding the garment. That shouldn't be possible. Her "
				...
				if (getSelf().human()) {
					c.write(getSelf(),
							target.name() + " yelps in surprise as you rip her " + article.getName() + " apart.");
				} else if (target.human()) {
				...
				if (getSelf().human()) {
					...
					c.write(getSelf(),
							getSelf().name() + " tries to cut your " + article.getName() + ", but fails to remove them.");
				}
				getSelf().consume(Item.MedicalSupplies,	1);
				return false;
			} else {
				if (getSelf().human()) {
					...
					c.write(getSelf(),
							getSelf().name() + " yanks on your " + article.getName() + ", but fails to remove it.");
				}
			}
			...
			if (!article.is(ClothingTrait.indestructible)
				...
				if (getSelf().human()) {
					...
					c.write(getSelf(),
							getSelf().name() + " yanks on your " + article.getName() + ", but fails to remove them.");
				}
				return false;
			}
		}
