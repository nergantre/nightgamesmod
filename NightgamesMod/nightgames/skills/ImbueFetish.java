...
public class ImbueFetish extends Skill {
	...
	public boolean usable(Combat c, Character target) {
		return getSelf().canAct() && c.getStance().mobile(getSelf())
				&& !c.getStance().prone(getSelf()) && !c.getStance().inserted();
	}
	...
	public String deal(Combat c, int damage, Result modifier,
			Character target) {
		return "You brandish one of your fetish needles - this one imbued with a "
				+ chosenFetish + " fetish - and stick it in " + target.name()
	...
	public String receive(Combat c, int damage, Result modifier,
		...
		return "You feel a tiny prick in your arm, and when you look you see a small"
				+ " needle sticking out. You remove the needle, but when you look back at "
				+ getSelf().name() + " - who has a maniacal look on "
				+ getSelf().possessivePronoun()
				+ " face - you feel an unnaturally strong attraction towards "
				+ getSelf().possessivePronoun() + " " + chosenFetish + ".";
	}
