...
public class AssFuck extends Fuck {
	...
	public boolean resolve(Combat c, Character target) {
		...
		if (!target.hasStatus(Stsflag.oiled)
				&& getSelf().getArousal().percent() > 50
				|| getSelf().has(Trait.alwaysready)
				|| getSelf().has(Trait.assmaster)) {
			...
			premessage += " up {other:possessive} ass with {self:possessive} "
					+ fluids + ".";
			target.add(c, new Oiled(target));
		} else if (!target.hasStatus(Stsflag.oiled)
				&& getSelf().has(Item.Lubricant)) {
			if (premessage.isEmpty()) {
				premessage = "{self:subject-action:lube|lubes}";
			} else {
		...
		if (getSelf().human()) {
			...
			} else if (getSelf().name().equals("Eve")
					&& c.getStance().behind(getSelf())) {
				m += 5;
				c.write(getSelf(), receive(c, premessage.length(),
		...
		if (getSelf().has(Trait.analFanatic) && getSelf().hasDick()) {
			c.write(getSelf(),
					String.format(
							"Now with %s %s deeply embedded within %s ass,"
	...
	public String deal(Combat c, int damage, Result modifier,
		...
		if (modifier == Result.normal) {
			...
			return target.name()
					+ "'s ass is oiled up and ready to go, but you're still too soft to penetrate her.";
		}
	}
