...
public class ShrinkRay extends Skill {
	...
	public boolean resolve(Combat c, Character target) {
		...
		if (getSelf().human()) {
			if (target.hasDick()) {
				c.write(getSelf(),
						deal(c, permanent ? 1 : 0, Result.special, target));
			} else {
				c.write(getSelf(),
			...
			if (target.hasDick()) {
				c.write(getSelf(),
						receive(c, permanent ? 1 : 0, Result.special, target));
			} else {
				c.write(getSelf(),
			...
			if (target.hasDick()) {
				...
				if (part != null) {
					target.body.temporaryAddOrReplacePartWithType(
							part.downgrade(), part, 10);
				} else {
					target.body.temporaryRemovePart(
							target.body.getRandom("cock"), 10);
				}
			} else {
				BreastsPart part = target.body
						.getBreastsAbove(BreastsPart.flat.size);
