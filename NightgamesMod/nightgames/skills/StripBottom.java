...
public class StripBottom extends Skill {
    ...
    public boolean resolve(Combat c, Character target) {
        ...
        if (getSelf().check(Attribute.Cunning, difficulty) || !target.canAct()) {
            ...
            if (getSelf().get(Attribute.Cunning) >= 30 && !target.crotchAvailable()
    ...
    public String receive(Combat c, int damage, Result modifier, Character target) {
        if (modifier == Result.miss) {
            ...
            String msg = getSelf().name() + " grabs the waistband of your " + stripped.getName() + " and pulls them down.";
            if (modifier == Result.critical) {
                msg += String.format(" Before you can react, %s also strips off your %s!", getSelf().name, extra.getName());
            }
            return msg;
        }
    }
