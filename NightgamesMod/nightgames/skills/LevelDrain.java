...
public class LevelDrain extends Drain {
	...
	public boolean resolve(Combat c, Character target) {
		...
		int type = Global.centeredrandom(2,
				getSelf().get(Attribute.Dark) / 20.0f, 2);
		if (getSelf().human()) {
		...
		switch (type) {
			...
			case 1:
				...
				if (stolen > 0) {
					...
					if (getSelf().human()) {
						c.write("You have absorbed " + stolen + " XP from "
								+ target.name() + "!\n");
					} else {
						c.write(getSelf().name() + " has absorbed " + stolen
								+ " XP from you!\n");
					}
				}
				break;
			case 2:
				...
				c.write(target.dong());
				if (getSelf().human()) {
					c.write("You have stolen a level from " + target.name()
							+ "'s levels and absorbed it as " + xpStolen
							+ " XP!\n");
				} else {
