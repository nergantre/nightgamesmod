...
public class Obey extends Skill {
	...
	public boolean usable(Combat c, Character target) {
		return getSelf().is(Stsflag.enthralled)
				&& !getSelf().is(Stsflag.stunned);
	}
	...
	public boolean resolve(Combat c, Character target) {
		if (getSelf().human()) {
			...
			c.write(getSelf(), getSelf().name()
					+ " stares ahead blankly, waiting for her orders.");
		}
		return true;
	}
	...
	@Override
	public String deal(Combat c, int paramInt, Result paramResult,
			Character paramCharacter) {
		return "";
	}

	@Override
	public String receive(Combat c, int paramInt, Result paramResult,
			Character paramCharacter) {
		return "";
	}
