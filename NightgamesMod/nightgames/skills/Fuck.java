...
public class Fuck extends Skill {
	...
	public boolean usable(Combat c, Character target) {
		return fuckable(c, target)
				&& (c.getStance().insert(getSelf(), getSelf()) != c
						.getStance()
	...
	String premessage(Combat c, Character target) {
		String premessage = "";
		Clothing underwear = getSelf().getOutfit()
				.getSlotAt(ClothingSlot.bottom, 0);
		Clothing bottom = getSelf().getOutfit().getSlotAt(ClothingSlot.bottom,
				1);
		...
		if (!bottomMessage.isEmpty() && getSelfOrgan().isType("cock")) {
			premessage = String.format(
					"{self:SUBJECT-ACTION:pull|pulls} down {self:possessive} %s halfway and ",
					bottomMessage);
	...
	public String deal(Combat c, int damage, Result modifier,
		...
		if (modifier == Result.normal) {
			return "you rub the head of your " + selfO.describe(getSelf())
					+ " around " + target.name()
					+ "'s entrance, causing her to shiver with anticipation. Once you're sufficiently lubricated "
					+ "with her wetness, you thrust into her "
			...
			if (!selfO.isReady(getSelf()) && !targetO.isReady(target)) {
				...
				return "you position your " + selfO.describe(getSelf())
						+ " at the entrance to " + target.name()
				...
				return "you're ready and willing to claim " + target.name()
						+ "'s eager "
						+ target.body.getRandomPussy().describe(target)
						+ ", but your shriveled " + selfO.describe(getSelf())
						+ " isn't cooperating. Maybe your self-control training has become "
						+ "too effective.";
			}
	...
	public String receive(Combat c, int damage, Result modifier,
		...
		BodyPart targetO = getTargetOrgan(target);
		if (modifier == Result.normal) {
			String message = getSelf().name() + " rubs her "
			...
			if (!selfO.isReady(getSelf()) || !targetO.isReady(target)) {
				...
				return (damage == 0 ? getSelf().name() + " " : "")
						+ "tries to push her " + selfO.describe(getSelf())
