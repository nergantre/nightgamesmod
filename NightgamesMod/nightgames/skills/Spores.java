...
public class Spores extends Skill {
	...
	public boolean resolve(Combat c, Character target) {
		if (target.wary()) {
			c.write(getSelf(),
					Global.format(
							"{self:SUBJECT-ACTION:release|releases} a mass of tiny particles, but "
			...
			c.write(getSelf(),
					Global.format(
							"{self:SUBJECT-ACTION:release|releases} a mass of tiny particles, and "
							+ "{other:subject-action:are|is} forced to breathe them in. The scent"
							+ " drives {other:pronoun} into a frenzy.",
							getSelf(), target));
