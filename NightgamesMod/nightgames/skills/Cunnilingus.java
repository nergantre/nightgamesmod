...
public class Cunnilingus extends Skill {
	...
	public boolean usable(Combat c, Character target) {
		...
		return target.crotchAvailable() && target.hasPussy()
	...
	public boolean resolve(Combat c, Character target) {
		...
		int i = 0;
		if (!facesitting && c.getStance().mobile(target)
				&& !target.roll(this, c, accuracy(c))) {
			results = Result.miss;
		} else {
			if (target.has(Trait.entrallingjuices) && Global.random(4) == 0
					&& !target.wary()) {
				i = -2;
	...
	public String deal(Combat c, int damage, Result modifier,
		...
		if (modifier == Result.miss) {
			return "You try to eat out " + target.name()
					+ ", but she pushes your head away.";
		}
		if (target.getArousal().get() < 10) {
			return "You run your tongue over " + target.name()
					+ "'s dry vulva, lubricating it with your saliva.";
		}
		if (modifier == Result.special) {
			return "Your skilled tongue explores " + target.name()
					...
					+ (damage == -1
							? " Under your skilled ministrations, her juices flow freely, and they unmistakably"
									+ " have their effect on you"
							: "")
					+ (damage == -2
							? " You feel a strange pull on you mind,"
		...
		if (target.getArousal().percent() > 80) {
			return "You relentlessly lick and suck the lips of " + target.name()
					...
					+ (damage == -1
							? " The highly aroused succubus' vulva is dripping with her "
		...
		if (r == 0) {
			return "You gently lick " + target.name()
					+ "'s pussy and sensitive clit."
					+ (damage == -1
							? " As you drink down her juices, they seem to flow "
		...
		return "You locate and capture " + target.name()
	...
	public String receive(Combat c, int damage, Result modifier,
		...
		if (modifier == Result.miss) {
			...
			return getSelf().nameOrPossessivePronoun()
					...
					+ (damage == -1
							? " Your aphrodisiac juices manage to arouse her as much as she aroused you."
							: "")
					+ (damage == -2
							? " Your tainted juices quickly reduce her into a willing thrall."
							: "");
		} else if (modifier == Result.reverse) {
			return getSelf().name()
					+ " obediently laps at your pussy as you sit on her face."
					+ (damage == -1
							? " Your aphrodisiac juices manage to arouse her as much as she aroused you."
							: "")
					+ (damage == -2
		...
		return getSelf().name()
				+ " locates and captures your clit between her lips and attacks it with her tongue."
				+ (damage == -1
						? " Your aphrodisiac juices manage to arouse her as much as she aroused you."
						: "")
				+ (damage == -2
