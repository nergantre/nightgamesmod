...
public class Offer extends Skill {
	...
	public String describe(Combat c) {
		...
		return other.hasDick() || other.has(Trait.strapped)
				? "Offer your " + (getSelf().hasPussy() ? "pussy" : "ass") + " to " + other.possessivePronoun() + "'s "
	...
	public String deal(Combat c, int damage, Result modifier, Character target) {
		...
		case miss:
			if (target.hasDick() || target.has(Trait.strapped)) {
				...
				return String.format("You wave your %s at %s, but %s ignores you completely.",
						getSelf().body.getRandomCock().describe(getSelf()), target.name(), target.pronoun());
			}
		case normal:
			return String.format(
		...
		case anal:
			return String.format(
					...
					target.body.getRandomInsertable().describe(target), Global.capitalizeFirstLetter(target.pronoun()));
		default: // special
			return String.format(
					...
					getSelf().body.getRandomPussy().describe(getSelf()), target.name(), target.possessivePronoun(),
					target.body.getRandomInsertable().describe(target), target.pronoun());
		}
	}
		...
		case miss:
			if (target.hasDick() || target.has(Trait.strapped)) {
				return String.format(
						...
						getSelf().name(), getSelf().directObject(), Global.capitalizeFirstLetter(getSelf().pronoun()),
						getSelf().pronoun());
			} else {
				return String.format(
