...
public class LeechEnergy extends Skill {
	...
	public boolean resolve(Combat c, Character target) {
		if(target.roll(this, c, accuracy(c))){
			...
			if (part.isType("hands")) {
				c.write(getSelf(), Global.format("{self:name-possessive} numerous " + selfPart.describe(getSelf())
							+ " latch onto {other:name-possessive} hands and swallow up {other:possessive} fingers. While the "
							+ selfPart.describe(getSelf()) + " are lasciviously licking {other:possessive} digits, " 
							+ "{other:subject-action:start|starts} feeling weak as {other:possessive} energy is being drained.", getSelf(), target));
				...
				+ selfPart.describe(getSelf()) + " are squeezing and pulling on {other:possessive} ankles, " 
				+ "{other:subject-action:start|starts} feeling weak as {other:possessive} energy is being drained through your toes.", getSelf(), target));
			} else if (part.isType("skin")) {
				c.write(getSelf(), Global.format("{self:name-possessive} numerous " + selfPart.describe(getSelf())
				+ " latch onto {other:name-possessive} body and coils around {other:possessive} waist. The numerous tips on the " + selfPart.describe(getSelf())
				...
				+ partStringSingular+ "-pussy churns against {other:possessive} cock relentlessly, sucking out both precum and {other:possessive} precious energy.", getSelf(), target));
				...
				+ partStringSingular+ "-mouth sucks and chews on {other:possessive} balls relentlessly, sucking out what little fight {other:subject-action:have|has}.", getSelf(), target));
				...
				+ partStringSingular+ "-cock thrusts in and out of {other:direct-object} relentlessly, draining {other:direct-object} of energy and replacing it with " + selfPart.getFluids(getSelf()) + ".", getSelf(), target));
			} else {
	...
	public String deal(Combat c, int damage, Result modifier, Character target) {
		if(modifier == Result.miss){
			BodyPart selfPart = getSelf().body.getRandom("tentacles");
			return "You try to drain energy with your " + selfPart.describe(getSelf()) + ", but " + target.name() + " dodges out of the way";
		}
		return "";
	}
	...
	public String receive(Combat c, int damage, Result modifier, Character target) {
		...
		if(modifier == Result.miss){
			return getSelf().name() + " tries to drain energy with her " + selfPart.describe(getSelf()) + ", but you dodge out of the way";
		}
		return "";
	}
