...
public class Shove extends Skill {
	...
	public boolean usable(Combat c, Character target) {
		...
		return !c.getStance().dom(getSelf()) && !c.getStance().prone(target)
	...
	public boolean resolve(Combat c, Character target) {
		...
		if (getSelf().get(Attribute.Ki) >= 1
			...
			target.pain(c,
					Global.random(10) + 15 + (getSelf().get(Attribute.Power)
							+ getSelf().get(Attribute.Ki)) / 4);
			if (getSelf().check(Attribute.Power,
					target.knockdownDC() - getSelf().get(Attribute.Ki))) {
				c.setStance(new Neutral(getSelf(), target));
			}
		} else if (c.getStance().getClass() == Mount.class
				|| c.getStance().getClass() == ReverseMount.class) {
			if (getSelf().check(Attribute.Power, target.knockdownDC() + 5)) {
				if (getSelf().human()) {
					...
					c.write(getSelf(), getSelf().name()
							+ " shoves you hard enough to free herself and jump up.");
				}
				c.setStance(new Neutral(getSelf(), target));
			} else {
				if (getSelf().human()) {
					c.write(getSelf(), "You push " + target.name()
							+ ", but you're unable to dislodge her.");
				} else if (target.human()) {
					c.write(getSelf(),
							getSelf().name() + " shoves you weakly.");
				}
				success = false;
			}
			target.pain(c,
					Global.random(10) + 10 + (getSelf().get(Attribute.Power)
							+ getSelf().get(Attribute.Ki)) / 4);
		} else {
			if (getSelf().check(Attribute.Power, target.knockdownDC())) {
				if (getSelf().human()) {
					c.write(getSelf(), "You shove " + target.name()
							+ " hard enough to knock her flat on her back.");
				} else if (target.human()) {
					c.write(getSelf(), getSelf().name()
							+ " knocks you off balance and you fall at her feet.");
				}
				target.add(c, new Falling(target));
			} else {
				if (getSelf().human()) {
					c.write(getSelf(), "You shove " + target.name()
							+ " back a step, but she keeps her footing.");
				} else if (target.human()) {
					c.write(getSelf(), getSelf().name()
							+ " pushes you back, but you're able to maintain your balance.");
				}
				success = false;
			}
			target.pain(c,
	...
	public String deal(Combat c, int damage, Result modifier,
			Character target) {
		return "You channel your ki into your hands and strike " + target.name()
				+ " in the chest, destroying her "
	...
	public String receive(Combat c, int damage, Result modifier,
			Character target) {
		return getSelf().name()
				+ " strikes you in the chest with her palm, staggering your footing. Suddenly your "
				+ target.getOutfit().getTopOfSlot(ClothingSlot.top).getName()
