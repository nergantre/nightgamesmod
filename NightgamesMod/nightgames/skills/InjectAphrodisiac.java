...
public class InjectAphrodisiac extends Skill {
	...
	public boolean resolve(Combat c, Character target) {
		...
		target.add(new Horny(target, magnitude, 10, "Aphrodisac Injection"));
		target.add(new InducedEuphoria(target));
	...
	public String deal(Combat c, int damage, Result modifier, Character target) {
		return Global.format("You quickly grab one of your syringes full of potent aphrodisiac before grabbing {other:name-do} and injecting {other:direct-object} with its contents. After you do so you see a bright flush spread across {other:possessive} face and {other:possessive} breathing picks up.",
				getSelf(), target);
	}

	public String receive(Combat c, int damage, Result modifier, Character target) {
		return Global.format("{self:SUBJECT} grins as {self:pronoun} flashes a hypodermic needle filled with light purple liquid. You yelp as {self:pronoun} grab your arm before jabbing you with the needle skillfully, pushing the plunger down to unload its cargo. A warmth floods through your body as the drug begins to take effect. It was an aphrodisiac!",
				getSelf(), target);
	}
