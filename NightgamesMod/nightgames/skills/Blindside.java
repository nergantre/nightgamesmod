	...
	public boolean resolve(Combat c, Character target) {
		if (getSelf().human()) {
			c.write(getSelf(), String.format(
					"You move up to %s and kiss %s strongly. " + "While %s is distracted, you throw %s down and plant "
							+ "yourself on top of %s.",
					target.name(), target.pronoun(), target.pronoun(), target.directObject(), target.directObject()));
		} else {
