...
public class Escape extends Skill {
	...
	public boolean usable(Combat c, Character target) {
		...
		return (c.getStance().sub(getSelf()) && !c.getStance().mobile(getSelf())
	...
	public boolean resolve(Combat c, Character target) {
		if (getSelf().bound()) {
			if (getSelf().check(Attribute.Cunning, 5 - getSelf().escape(c))) {
				if (getSelf().human()) {
					c.write(getSelf(),
							"You slip your hands out of your restraints.");
				} else if (target.human()) {
					c.write(getSelf(),
							getSelf().name() + " manages to free herself.");
				}
				getSelf().free();
			} else {
				if (getSelf().human()) {
					c.write(getSelf(),
							"You try to slip your restraints, but can't get free.");
				} else if (target.human()) {
					c.write(getSelf(), getSelf().name()
							+ " squirms against her restraints fruitlessly.");
				}
				getSelf().struggle();
				return false;
			}
		} else if (getSelf().check(Attribute.Cunning,
				10 + target.get(Attribute.Cunning) - getSelf().escape(c))) {
			if (getSelf().human()) {
				if (getSelf().hasStatus(Stsflag.cockbound)) {
					c.write(getSelf(), "You somehow managed to wiggle out of "
							+ target.name() + "'s iron grip on your dick.");
					getSelf().removeStatus(Stsflag.cockbound);
					return true;
				}
				c.write(getSelf(), "Your quick wits find a gap in "
						+ target.name() + "'s hold and you slip away.");
			} else if (target.human()) {
				if (getSelf().hasStatus(Stsflag.cockbound)) {
					c.write(getSelf(),
							"She somehow managed to wiggle out of your iron grip on her dick.");
					getSelf().removeStatus(Stsflag.cockbound);
					return true;
				}
				c.write(getSelf(),
						getSelf().name()
								+ " goes limp and you take the opportunity to adjust your grip on her. As soon as you move, she bolts out of your weakened hold. "
								+ "It was a trick!");
			}
