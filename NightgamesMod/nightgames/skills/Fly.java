...
public class Fly extends Fuck {
	...
	public boolean requirements(Combat c, Character user, Character target) {
		return user.body.get("wings").size() > 0
				&& user.get(Attribute.Power) >= 15;
	}

	@Override
	public boolean usable(Combat c, Character target) {
		return fuckable(c, target) && !target.wary() && getSelf().canAct()
	...
	public boolean resolve(Combat c, Character target) {
		...
		if (getSelf().human()) {
			c.write(getSelf(),
					premessage + deal(c, premessage.length(), result, target));
		} else if (target.human()) {
			c.write(getSelf(), premessage
					+ receive(c, premessage.length(), result, getSelf()));
		}
	...
	public String deal(Combat c, int amount, Result modifier,
		...
		if (modifier == Result.miss) {
			return (amount == 0 ? "You " : "") + "grab " + target.name()
