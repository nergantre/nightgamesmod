...
public class Strapon extends Skill {
	...
	public boolean usable(Combat c, Character target) {
		return getSelf().canAct() && !getSelf().has(Trait.strapped) && c.getStance().mobile(getSelf())
				&& !c.getStance().prone(getSelf()) && (getSelf().has(Item.Strapon) || getSelf().has(Item.Strapon2))
				&& !getSelf().hasDick() && !c.getStance().connected()
	...
	public boolean resolve(Combat c, Character target) {
		...
		if (unequipped.isEmpty()) {
			...
			if (getSelf().human()) {
				c.write(getSelf(),
						"You take off your " + unequipped.get(0) + " and fasten a strap on dildo onto yourself.");
			} else {
				c.write(getSelf(),
