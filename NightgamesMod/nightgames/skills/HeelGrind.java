...
public class HeelGrind extends Skill {
	...
	public boolean usable(Combat c, Character target) {
		return c.getStance().behind(getSelf()) && target.crotchAvailable()
				&& getSelf().canAct()
				&& !c.getStance().vaginallyPenetrated(target)
				&& target.hasPussy() && getSelf().outfit.hasNoShoes();
	}
	...
	public boolean resolve(Combat c, Character target) {
		...
		target.body.pleasure(getSelf(), getSelf().body.getRandom("feet"),
				target.body.getRandom("pussy"), m, c);
		target.body.pleasure(getSelf(), getSelf().body.getRandom("hands"),
				target.body.getRandom("breasts"), m2, c);
