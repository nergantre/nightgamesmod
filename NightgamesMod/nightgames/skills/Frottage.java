...
public class Frottage extends Skill {
	...
	public boolean usable(Combat c, Character target) {
		return getSelf().canAct() && c.getStance().mobile(getSelf())
				&& !c.getStance().sub(getSelf()) && !c.getStance().havingSex()
				&& target.crotchAvailable()
				&& (getSelf().hasDick() && getSelf().crotchAvailable()
	...
	public boolean resolve(Combat c, Character target) {
		...
		BodyPart receiver = target.hasDick() ? target.body.getRandomCock()
				: target.body.getRandomPussy();
		BodyPart dealer = getSelf().hasDick() ? getSelf().body.getRandomCock()
	...
	public String deal(Combat c, int damage, Result modifier,
		...
		if (modifier == Result.special) {
			return "You tease " + target.name()
					+ "'s penis with your own, dueling her like a pair of fencers.";
		} else {
