...
public class Aphrodisiac extends Skill {
	...
	public String deal(Combat c, int damage, Result modifier,
		...
		if (modifier == Result.special) {
			return "You pop an Aphrodisiac into your Aerosolizer and spray "
					+ target.name()
			...
			return getSelf().subjectAction("dip", "dips") + " a finger "
					...
					+ " juices" + " on " + getSelf().possessivePronoun()
					+ " fingertip, " + getSelf().subjectAction("pull", "pulls")
					+ " it out and flicks it at " + target.directObject() + ","
					+ " skillfully depositing it in "
					+ target.possessivePronoun() + " open mouth. "
					+ Global.capitalizeFirstLetter(target.subject())
					+ " immediately feel" + " a flash of heat spread through "
					+ target.directObject() + " and only a small part of it"
					+ " results from the anger caused by "
	...
	public String receive(Combat c, int damage, Result modifier,
		...
		if (modifier == Result.miss) {
			return getSelf().subjectAction("splash", "splashes")
					...
					+ " direction, but none of it hits you.";
