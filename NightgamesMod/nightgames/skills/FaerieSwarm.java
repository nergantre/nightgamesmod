...
public class FaerieSwarm extends Skill {
	...
	public boolean resolve(Combat c, Character target) {
		...
		if (target.getOutfit().isNude()) {
			...
			target.body.pleasure(getSelf(), null, null, 25 + Global.random(getSelf().get(Attribute.Arcane)), c);
		} else {
			if (getSelf().human()) {
	...
	public String deal(Combat c, int damage, Result modifier, Character target) {
		...
		"You unroll the summoning scroll and unleash a cloud of cute, naked faeries. "
				...
				+ Global.capitalizeFirstLetter(target.pronoun()) + " tries in vain to defend " + target.directObject()
				+ "self, but there are too many of them and they're too quick. "
				+ Global.capitalizeFirstLetter(target.pronoun())
				+ "'s reduced to writhing and giggling in pleasure until " + "the brief summoning spell exires.";
	}
