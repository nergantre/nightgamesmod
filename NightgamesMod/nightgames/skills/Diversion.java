...
public class Diversion extends Skill {
	...
	public boolean usable(Combat c, Character target) {
		return !target.wary() && getSelf().canAct()
				&& c.getStance().mobile(getSelf()) && c.getStance().facing()
				&& !getSelf().torsoNude() && !c.getStance().prone(getSelf())
	...
	public String deal(Combat c, int damage, Result modifier,
		...
		if (modifier == Result.normal) {
			...
			return "You quickly strip off your " + article.getName()
					+ " and throw it to the right, while you jump to the left. "
					+ target.getName() + " catches your discarded clothing, "
	...
	public String receive(Combat c, int damage, Result modifier,
		...
		if (modifier == Result.normal) {
			...
			return "You lose sight of " + getSelf().name()
					...
					+ "but you find yourself holding just her "
					+ article.getName() + ". Wait... what the fuck?";
		} else {
			// TODO this is kind of terrible (deal does the effect...)
			Clothing article = getSelf().strip(ClothingSlot.bottom, c);
			return "You lose sight of " + getSelf().name()
