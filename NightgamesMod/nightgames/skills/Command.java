...
public class Command extends Skill {
	...
	public boolean resolve(Combat c, Character target) {
		...
		oral.retainAll(
				Arrays.asList(CommandType.GIVE_ANNILINGUS, CommandType.GIVE_BLOWJOB, CommandType.GIVE_CUNNILINGUS));
	...
	private EnumSet<CommandType> availableCommands(Combat c, Character target) {
		...
		if (c.getStance().dom(getSelf()) && c.getStance().en == Stance.standingover) {
		...
		if (Global.getByTactics(c, Tactics.fucking).stream().map(s -> s.copy(getSelf()))
	...
	private void executeCommand(CommandType chosen, Combat c, Character target) {
		...
		case GIVE_ANNILINGUS:
			c.write(getSelf(),
					String.format(
							"%s presents %s ass to you, and you" + " instantly dive towards it and lick it fervently.",
		...
		case GIVE_BLOWJOB:
			c.write(getSelf(), String.format(
					"%s holds up %s %s, and you simply can't resist"
							+ " the tantilizing appendage. You lower your head and lick and suck" + " it all over.",
					getSelf().name(), getSelf().possessivePronoun(),
		...
		case GIVE_CUNNILINGUS:
			c.write(getSelf(), String.format(
					"%s spreads %s labia and before %s can"
							+ " even tell you what to do, you are already between %s legs" + " slavering away at it.",
		...
		case MASTER_BEHIND:
			c.write(getSelf(),
					String.format(
							"Freezing you in place with a mere"
									+ " glance, %s casually walks around you and grabs you from" + " behind.",
							getSelf().name()));
			...
			c.write(getSelf(),
					String.format(
							"%s fixes you with an intense glare, telling"
									...
									+ " your thoughts as %s turns away from you and sits down on your" + " belly.",
		...
		case MASTER_STRAPON:
			c.write(getSelf(),
					String.format(
							"%s affixes an impressive-looking strapon"
									...
									+ " tells you that you like the look of it, you are practically" + " salivating.",
							getSelf().name(), getSelf().possessivePronoun(), getSelf().pronoun()));
			...
			break;
		case MASTURBATE:
			BodyPart pleasured = target.body.getRandom(target.hasDick() ? "cock" : target.hasPussy() ? "pussy" : "ass");
			c.write(getSelf(),
					String.format("Feeling a bit uninspired, %s just tells you" + " to play with your %s for %s.",
							getSelf().name(), pleasured.describe(target), getSelf().directObject()));
			target.body.pleasure(target, target.body.getRandom("hands"), pleasured, 10 + Global.random(20), c);
			break;
		case HURT_SELF:
			c.write(getSelf(), String.format(
					"Following a voiceless command," + " you slam your elbow into you gut as hard as you can."
							+ " It hurts, but the look of pure amusement on %s face" + " makes everything alright.",
		...
		case WORSHIP_COCK:
			c.write(getSelf(), String.format(
					...
					getSelf().name(), getSelf().possessivePronoun(), getSelf().pronoun(), getSelf().possessivePronoun(),
		...
		case MASTER_INSERT:
			c.write(getSelf(),
					String.format(
							...
							getSelf().name(), getSelf().pronoun()));
			Global.getByTactics(c, Tactics.fucking).stream().map(s -> s.copy(getSelf()))
					.filter(s -> s.requirements(c, getSelf(), target) && s.usable(c, target)).findAny().get()
					.resolve(c, target);
			break;
		case MASTER_FACESIT:
	...
	private enum CommandType {
		STRIP_MASTER, STRIP_SLAVE, WORSHIP_COCK, WORSHIP_PUSSY, GIVE_BLOWJOB, GIVE_CUNNILINGUS, GIVE_ANNILINGUS, MASTURBATE, HURT_SELF, SUBMIT, MASTER_MOUNT, MASTER_FACESIT, MASTER_REVERSE_MOUNT, MASTER_BEHIND, MASTER_STRAPON, MASTER_INSERT
	}
}
