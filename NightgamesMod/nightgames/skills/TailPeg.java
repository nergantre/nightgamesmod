...
public class TailPeg extends Skill {
	...
	public boolean requirements(Combat c, Character user, Character target) {
		...
		boolean hasFuckableTail = tails.stream()
				.anyMatch(p -> p.isType("tail") && p != TailPart.cat);
		return hasFuckableTail && (user.get(Attribute.Dark) >= 1
				|| user.get(Attribute.Seduction) >= 20);
	}
	...
	public boolean resolve(Combat c, Character target) {
		if (target.roll(this, c,
			...
			boolean vaginal = c.getStance()
					.anallyPenetrated(c.getOther(getSelf()));
			...
			if (target.human()) {
				if (vaginal) {
					c.write(getSelf(),
							receive(c, 0, Result.intercourse, target));
			...
			if (c.getStance().havingSex()) {
				if (vaginal) {
					target.body.pleasure(getSelf(),
							...
							target.body.getRandom("pussy"), strength, c);
					target.add(c, new TailFucked(target, getSelf(), "pussy"));
				} else {
					target.body.pleasure(getSelf(),
							getSelf().body.getRandom("tail"),
							target.body.getRandom("ass"), strength, c);
		...
		switch (modifier) {
			case critical:
				return "You flex your prehensile tail and spread "
						+ target.nameOrPossessivePronoun() + " legs apart. "
						+ "You quickly lube it up with "
						+ target.possessivePronoun()
						+ " juices and slide it into her ass and start pumping.";
			case miss:
				return "You try to peg " + target.name()
						+ " with your tail, but " + target.pronoun()
						+ " manages to clench " + target.possessivePronoun()
						+ " butt cheeks together in time to keep you out.";
			case normal:
				return "You move towards " + target.name() + " and hold "
						+ target.possessivePronoun() + " hands above "
						+ target.possessivePronoun()
			...
			case special:
				return "You smile down at " + target.name()
						+ " and move your flexible tail behind "
						+ target.directObject() + ". You spread "
						+ target.possessivePronoun()
						+ " cheeks with your tail and plunge it into "
						+ target.possessivePronoun() + " tight pucker. "
						+ target.name()
			...
			case intercourse:
				return "You smile down at " + target.name()
						...
						+ target.possessivePronoun() + " wet slit. "
						+ target.name()
						+ " moans loudly at the sudden intrusion.";
			case strong:
				if (target.body.getLargestBreasts().size >= 2) {
					...
					return "You hug " + target.name()
							...
							+ target.possessivePronoun() + " ass, and tickle "
							+ target.possessivePronoun()
