...
public class FTCMatch extends Match {
	...
	private void buildFTCMap(Character north, Character west, Character south, Character east, Character prey) {
		...
		Area nBase = new Area("North Base",
				String.format("You are in a small camp on the northern edge of the forest. " + "%s has %s base here.",
						north.subjectAction("have", "has"), north.possessivePronoun()),
				Movement.ftcNorthBase);
		Area wBase = new Area("West Base",
				String.format("You are in a small camp on the western edge of the forest. " + "%s has %s base here.",
						west.subjectAction("have", "has"), west.possessivePronoun()),
				Movement.ftcWestBase);
		Area sBase = new Area("South Base",
				String.format("You are in a small camp on the southern edge of the forest. " + "%s has %s base here.",
						south.subjectAction("have", "has"), south.possessivePronoun()),
				Movement.ftcSouthBase);
		Area eBase = new Area("East Base",
				String.format("You are in a small camp on the eastern edge of the forest. " + "%s has %s base here.",
		...
		Area pBase = new Area("Central Camp",
				String.format(
						"You are in a clearing in the middle of the forest. There are no"
								+ " trees here, just a small camp where %s can " + "get a new Flag if it gets captured",
		...
		Area pond = new Area("Small Pond",
				"You are at the edge of a small pond surrounded"
						+ " by shrubbery. You could imagine taking a quick dip here, but it's a" + " little risky.",
				Movement.ftcPond);
		...
		Area trail = new Area("Trail",
				"You are following a trail along some relatively"
						+ " short trees. If you've got the upper body strength, you could" + " probably climb up one.",
				Movement.ftcTrail);
		Area lodge = new Area("Lodge",
				"You are in a quaint wooden lodge. There are numerous"
						+ " herbs and chemicals here, and you should be able to mix up some good" + " stuff.",
				Movement.ftcLodge);
	...
	public void turnInFlag(Character ch) {
		flagInCenter = true;
		score(ch, 5);
		Global.gui().message(Global.format(
				"<b>{self:SUBJECT-ACTION:turn|turns} in the flag and " + "{self:action:gain|gains} five points.</b>",
	...
	public void grabFlag() {
		...
		flagCounter = 0;
		Global.gui()
				.message(Global.format("{self:SUBJECT-ACTION:grab|grabs} a new flag from the stash. That means"
