...
public class TreeAmbush extends Action {
	...
	public boolean usable(Character user) {
		return user.location().id() == Movement.ftcTrail
				&& (user.get(Attribute.Power) >= 20 || user.get(Attribute.Animism) >= 10) && user.state != State.inTree;
	}
	...
	public Movement execute(Character user) {
		if (user.human()) {
			if (user.get(Attribute.Animism) >= 10) {
				Global.gui()
						.message("Following your instincts, you clamber up a tree" + " to await a unwitting passerby.");
			} else {
				Global.gui()
						.message("You climb up a tree that has a branch hanging over"
