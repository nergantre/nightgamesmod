...
public class MaraTime extends BaseNPCTime {
    ...
    public void subVisitIntro(String choice) {
        if (npc.getAffection(player) > 0) {
            Global.gui().message(
                            "You go to the computer lab to find Mara for some quality time. She immediately breaks into a smile when she sees you enter. You can tell "
            ...
            Global.gui().message(
                            "You eventually find Mara in one of the computer labs, or at least a room labeled Computer Lab D. You typically think of a computer "
            ...
            Global.gui().message(
                            "You head to the computer lab to see Mara. She jumps in surprise when the door opens, but relaxes when she sees who you are. <i>\"Hi "
                                            + player.name() + ". "
        ...
        if (choice.equals("Sex")) {
            if (npc.getAffection(player) >= 8 && (!player.has(Trait.ticklemonster) || Global.random(2) == 1)) {
                ...
                if (!player.has(Trait.ticklemonster)) {
                    Global.gui().message(
                                    "<p><b>You've gotten better at finding sensitive spots when tickling nude opponents.</b>");
                    player.add(Trait.ticklemonster);
                    npc.add(Trait.ticklemonster);
                }
            } else {
                Global.gui().message(
                                "You suggest finding a bed for some private fun. Mara smiles at you mischievously. <i>\"We don't really need a bed for that\"</i> She clears some space "
            ...
            if (npc.getAffection(player) >= 16 && (!player.has(Trait.spider) || Global.random(2) == 1)) {
                ...
                if (!player.has(Trait.spider)) {
                    Global.gui().message(
                                    "<p><b>Mara has taught you to make the brilliant and insanely complex Spiderweb Trap.</b>");
                    player.add(Trait.spider);
                    npc.add(Trait.spider);
                }
            } else {
                Global.gui().message(
                                "You knew Mara was clever, but you didn't expect playing a board game against her to be this frustrating. Every move you make, she seems to have planned far "
            ...
            if (npc.getAffection(player) >= 12 && (!player.has(Trait.heeldrop) || Global.random(2) == 1)) {
                ...
                if (!player.has(Trait.heeldrop)) {
                    Global.gui().message(
                                    "<p><b>You've experienced Mara's most painful technique and learned how to use it yourself.</b>");
                    player.add(Trait.heeldrop);
                    npc.add(Trait.heeldrop);
                }
            } else {
                Global.gui().message(
                                "You suggest heading to the gym to do a bit of sparring, but she suggests her room would give you more privacy. This turns out to be important, because when "
