...
public class MagicTraining extends Activity {
		...
		if (!Global.checkFlag(Flag.metAisha)) {
			Global.gui().message(
					"Aisha apparently spends most of her time in a mostly abandoned creative writing reference room in the back of the liberal arts building. On paper, she "
							...
							+ "gives you a gentle, charming smile. <i>\"Hello "
							+ player.name()
			...
			Global.gui().choose(this, "Lesson: $"
					+ 1000 * (player.getPure(Attribute.Arcane) + 1));
			...
			if (player.money >= 500
					+ 500 * (player.getPure(Attribute.Animism) + 1)) {
				player.money -= 500
						+ 500 * (player.getPure(Attribute.Animism) + 1);
				Global.gui().message(
						"Kat comes in again to help you practice tapping "
			...
			Global.gui().message(
					"You bring up the topic of Kat's animal spirit and "
			...
			Global.gui().choose(this, "Lesson: $"
					+ (500 + 500 * (player.getPure(Attribute.Arcane) + 1)));
			Global.gui().choose(this, "Leave");
		} else if (choice.startsWith("Get Animal Spirit")) {
			Global.gui().message(
					"Kat agrees to come to the creative writing reference"
			...
			Global.gui().message(
					"You purchase a minor scroll. With the correct spell, "
	...
	public void shop(Character npc, int budget) {
		if (npc.getPure(Attribute.Arcane) > 0
	...
	private void presentOptions() {
		Global.gui().choose(this,
				"Lesson: $" + 1000 * (player.getPure(Attribute.Arcane) + 1));
		if (player.getPure(Attribute.Animism) >= 1) {
			Global.gui().choose(this, "Animism training: $"
					+ (500 + 500 * (player.getPure(Attribute.Animism) + 1)));
		}
		if (Global.checkFlag(Flag.catspirit) && !Global.checkFlag(Flag.furry)) {
			Global.gui().choose(this, "Ask about Animal Spirit");
		}
		if (Global.checkFlag(Flag.furry)
				&& player.getPure(Attribute.Animism) == 0) {
			Global.gui().choose(this, "Get Animal Spirit");
		}
