...
public class Daytime {
    ...
    private boolean morning() {
        ...
        if (MagicMilkAddiction.getMagicMilkAddictionLevel(player) >= 5) {
            Global.gui().message(
                            "You wake up in the morning with a burning need for Cassie's milk. The thought of resisting the urge doesn't even enter your mind. You quickly whip out your cellphone and dial Cassie's number. "
            ...
            player.setFlag(MagicMilkAddiction.MAGICMILK_ADDICTION_FLAG, player.getFlag(MagicMilkAddiction.MAGICMILK_ADDICTION_FLAG) + 2);
        } else if (MagicMilkAddiction.getMagicMilkAddictionLevel(player) >= 4) {
            Global.gui().message(
                            "When you wake up in the morning, the first thing you think of is Cassie's breasts. And the second. And the third. In fact, you realize that's all you can think of right now. "
            ...
            Global.gui().message(
                            "You wake up in the morning with damp underwear. You realize that you've been dreaming of Cassie's milk the entire night. This can't be healthy... <br>"
            ...
            Global.gui().message(
                            "You wake up in the morning with your throat feeling strangely parched. You step into the kitchen and take out a carton of milk to attempt to slake your thirst. "
        ...
        if (player.getLevel() >= 10 && player.getRank() == 0) {
            ...
            Global.gui().message(
                            "You try to get as much sleep as you can before your morning classes.<p>You're done with classes by mid-afternoon and have the rest of the day free.");
            time = 15;
        }
        return false;
    }
    ...
    public void plan() {
        ...
        if (time < 22) {
            ...
            player.getWillpower().setTemporaryMax(-1);
            if (player.getFlag(MagicMilkAddiction.MAGICMILK_DRANK_DAYTIME_FLAG) != 0) {
                player.setFlag(MagicMilkAddiction.MAGICMILK_DRANK_DAYTIME_FLAG, 0);
            } else if (magicmilk_addiction > 0) {
                player.getWillpower().setTemporaryMax(player.getWillpower().max() / MagicMilkAddiction.getMagicMilkAddictionLevel(player));
                player.setFlag(MagicMilkAddiction.MAGICMILK_ADDICTION_FLAG, Math.max(0, magicmilk_addiction - 3));
                Global.gui().message("You're in withdraw from abstaining from drinking Cassie's milk. You will have lowered willpower in tonight's match...");
            }
