...
public abstract class BaseNPCTime extends Activity {
	...
	public void visit(String choice) {
		...
		Optional<TransformationOption> optionalOption = options.stream()
				.filter(opt -> choice.equals(opt.option)).findFirst();
		Optional<Loot> optionalGiftOption = giftables.stream()
				.filter(gift -> choice
						.equals(Global.capitalizeFirstLetter(gift.getName())))
				.findFirst();

		if (optionalOption.isPresent()) {
			TransformationOption option = optionalOption.get();
			boolean hasAll = option.ingredients.entrySet().stream().allMatch(
					entry -> player.has(entry.getKey(), entry.getValue()));
			if (hasAll) {
				Global.gui().message(Global.format(option.scene, npc, player));
				option.ingredients.entrySet().stream().forEach(entry -> player
						.consume(entry.getKey(), entry.getValue(), false));
				option.effect.execute(null, player, npc);
				Global.gui().choose(this, "Leave");
			} else {
				Global.gui()
						.message(Global.format(noRequestedItems, npc, player));
			...
			giftables.stream().forEach(loot -> Global.gui().choose(this,
					Global.capitalizeFirstLetter(loot.getName())));
			Global.gui().choose(this, "Back");
			...
			Global.gui()
					.message(Global.format(transformationIntro, npc, player));
			...
			options.forEach(opt -> {
				if (opt.requirements.stream()
						.allMatch(req -> req.meets(null, player, npc))) {
			...
			if (npc.getAffection(player) > 25
					&& (advTrait == null || npc.has(advTrait))) {
