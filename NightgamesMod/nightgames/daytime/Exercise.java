...
public class Exercise extends Activity {
	...
	private void showScene(Scene chosen) { 
	     ...
	     case basic3: 
	       ...
	       break;
	     case fail1: 
	       Global.gui().message("Maybe you didn't stretch well enough before you started, but a few minutes into your run you feel like you've pulled a muscle in your leg. Better take a break rather than injure yourself before the match.");
	       break;
	     case cassie1: 
	       Global.gui().message("You head over to the campus gym and coincidentally run into Cassie there. <i>\"Hi. I'm not really much of a fitness enthusiast, but I need to get into better shape if I'm going to stay competitive.\"</i><br>The two of you spend some time doing light exercise and chatting.");
	       
	       Global.getNPC("Cassie").gainAffection(this.player, 1);
	       this.player.gainAffection(Global.getNPC("Cassie"), 1);
	       break;
	     case jewel1: 
	       Global.gui().message("You're going for a run around the campus and run into Jewel doing the same. She makes an immediate beeline towards you. <i>\"You're not getting out of running today, no matter how tempting the alternative is. We're going to get some real exercise.\"</i> She pushes you a lot harder than you had planned and you're exhausted by the end of it, but you did manage to keep up with her.");
	       
	       Global.getNPC("Jewel").gainAffection(this.player, 1);
	       this.player.gainAffection(Global.getNPC("Jewel"), 1);
	     }
	   }
	   ...
	   private Scene pickScene(int gain) {
	     ...
	     if (gain == 1) {
	       ...
	       if (Global.getNPC("Cassie").getAffection(this.player) >= 5) {
	         available.add(Scene.cassie1);
	       }
	       if ((Global.getNPC("Jewel").getAffection(this.player) >= 5) && (this.player.getStamina().max() >= 35)) {
