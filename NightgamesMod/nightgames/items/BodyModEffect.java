		...
		switch (effect) {
				...
				if (original != null) {
					...
					if (newPart == original) {
						boolean eventful = user.body
								.temporaryAddOrReplacePartWithType(newPart,
										original, duration);
						message = eventful ? Global.format(
								String.format(
										"{self:NAME-POSSESSIVE} %s was reenforced",
										original.fullDescribe(user)),
								user, opponent) : "";
					} else {
						user.body.temporaryAddOrReplacePartWithType(newPart,
								original, duration);
						message = Global.format(
								String.format(
										"{self:NAME-POSSESSIVE} %s grew into %s",
										original.fullDescribe(user),
										Global.prependPrefix(newPart.prefix(),
			...
			case downgrade:
				if (original != null) {
					...
					if (newPart == original) {
						...
						message = Global.format(
								String.format(
										"{self:NAME-POSSESSIVE} %s shrunk into %s",
										original.fullDescribe(user),
										Global.prependPrefix(newPart.prefix(),
												newPart.fullDescribe(user))),
								user, opponent);
			...
			case replace:
				if (original == affected) {
					boolean eventful = user.body
							.temporaryAddOrReplacePartWithType(affected,
									original, duration);
					message = eventful ? Global.format(
							String.format(
									"{self:NAME-POSSESSIVE} %s was reenforced",
					...
					user.body.temporaryAddOrReplacePartWithType(affected,
							original, duration);
					message = Global.format(
							String.format(
									"{self:NAME-POSSESSIVE} %s turned into %s",
									original.fullDescribe(user),
									Global.prependPrefix(affected.prefix(),
			...
			case growplus:
				if (original == null) {
					...
					if (newPart == original) {
						...
						user.body.temporaryAddOrReplacePartWithType(newPart,
								original, duration);
						message = Global.format(String.format(
								"{self:NAME-POSSESSIVE} %s grew into %s%s",
								original.fullDescribe(user), newPart.prefix(),
								newPart.fullDescribe(user)), user, opponent);
					}
				}
			default:
		}
