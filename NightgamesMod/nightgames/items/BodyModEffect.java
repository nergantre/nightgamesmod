...
public class BodyModEffect extends ItemEffect {
	...
	public enum Effect {
		upgrade, downgrade, replace, grow, growMultiple, growplus,
	}
	...
	public boolean use(Combat c, Character user, Character opponent, Item item) {
		...
		case upgrade:
			if (original != null) {
				...
				if (newPart == original) {
					...
					message = eventful ? Global.format(
							String.format("{self:NAME-POSSESSIVE} %s was reenforced", original.fullDescribe(user)),
							user, opponent) : "";
				} else {
					user.body.temporaryAddOrReplacePartWithType(newPart, original, duration);
					message = Global.format(
							String.format("{self:NAME-POSSESSIVE} %s grew into %s", original.fullDescribe(user),
									Global.prependPrefix(newPart.prefix(), newPart.fullDescribe(user))),
							user, opponent);
				}
			} else {
				message = "";
		...
		case downgrade:
			if (original != null) {
				...
				if (newPart == original) {
					...
					user.body.temporaryAddOrReplacePartWithType(newPart, original, duration);
					message = Global.format(
							String.format("{self:NAME-POSSESSIVE} %s shrunk into %s", original.fullDescribe(user),
									Global.prependPrefix(newPart.prefix(), newPart.fullDescribe(user))),
							user, opponent);
				}
			} else {
		...
		case replace:
			if (original == affected) {
				...
				message = eventful ? Global.format(
						String.format("{self:NAME-POSSESSIVE} %s was reenforced", original.fullDescribe(user)), user,
						opponent) : "";
			} else if (original != null) {
				user.body.temporaryAddOrReplacePartWithType(affected, original, duration);
				message = Global
						.format(String.format("{self:NAME-POSSESSIVE} %s turned into %s", original.fullDescribe(user),
								Global.prependPrefix(affected.prefix(), affected.fullDescribe(user))), user, opponent);
			} else {
			...
			if (original == null) {
				...
				if (newPart == original) {
					...
					message = Global.format(String.format("{self:NAME-POSSESSIVE} %s grew into %s%s",
							original.fullDescribe(user), newPart.prefix(), newPart.fullDescribe(user)), user, opponent);
				}
			}
		default:
		}
