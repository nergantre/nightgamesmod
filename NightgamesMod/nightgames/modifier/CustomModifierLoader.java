...
public final class CustomModifierLoader {
    ...
    private static ActionModifier readActionModifier(JSONObject obj) {
        ...
        ModifierComponent<? extends ActionModifier> template =
                        (ModifierComponent<? extends ActionModifier>) ActionModifier.TYPES.stream()
                                        .filter(t -> ((ModifierComponent<? extends ActionModifier>) t).name()
                                                        .equals(type))
    ...
    private static SkillModifier readSkillModifier(JSONObject obj) {
        String type = JSONUtils.readString(obj, "type");
        ModifierComponent<? extends SkillModifier> template =
                        (ModifierComponent<? extends SkillModifier>) SkillModifier.TYPES.stream()
                                        .filter(t -> ((ModifierComponent<? extends SkillModifier>) t).name()
                                                        .equals(type))
                                        .findAny().orElseThrow(() -> new IllegalArgumentException(
                                                        "Unkown modifier type " + type));
        if (!obj.containsKey("value") || !(obj.get("value") instanceof JSONObject)) {
    ...
    private static ClothingModifier readClothingModifier(JSONObject obj) {
        String type = JSONUtils.readString(obj, "type");
        ModifierComponent<? extends ClothingModifier> template =
                        (ModifierComponent<? extends ClothingModifier>) ClothingModifier.TYPES.stream()
                                        .filter(t -> ((ModifierComponent<? extends ClothingModifier>) t).name()
                                                        .equals(type))
                                        .findAny().orElseThrow(() -> new IllegalArgumentException(
                                                        "Unkown modifier type " + type));
        if (!obj.containsKey("value") || !(obj.get("value") instanceof JSONObject)) {
    ...
    private static ItemModifier readItemModifier(JSONObject obj) {
        String type = JSONUtils.readString(obj, "type");
        ModifierComponent<? extends ItemModifier> template =
                        (ModifierComponent<? extends ItemModifier>) ItemModifier.TYPES.stream()
                                        .filter(t -> ((ModifierComponent<? extends ItemModifier>) t).name()
                                                        .equals(type))
                                        .findAny().orElseThrow(() -> new IllegalArgumentException(
                                                        "Unkown modifier type " + type));
        if (!obj.containsKey("value") || !(obj.get("value") instanceof JSONObject)) {
