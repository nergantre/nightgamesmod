...
public class FairyFem extends Pet {
			...
			switch (Global.random(4)) {
				case 3:
					if (target.crotchAvailable()) {
						...
						c.write(owner(), own()
								+ "faerie flies around the edge of the fight looking for an opening.");
					}
					break;
				case 2:
				...
				case 1:
					c.write(owner(),
							own() + "faerie flies around " + owner().name()
									+ ", channelling energy into her.");
					owner().buildMojo(c, 20 + power);
					break;
				default:
					c.write(owner(), own() + "faerie rains magic energy on "
							+ owner().name() + ", restoring her strength.");
			...
			switch (Global.random(4)) {
				case 3:
					if (target.breastsAvailable()) {
						...
						target.body.pleasure(null, null,
								target.body.getRandom("breasts"),
								3 + 2 * Global.random(power), c);
						target.pain(c, 3 + 2 * Global.random(power), false,
								true);
					} else {
						c.write(owner(), own()
	...
	public void vanquish(Combat c, Pet opponent) {
		switch (opponent.type()) {
			case fairyfem:
				c.write(owner(),
						"The two faeries circle around each other vying for the upper hand. "
								+ own() + "faerie catches " + opponent.own()
								+ "faerie by the hips and starts to eat her out. "
								+ opponent.own()
								+ " fae struggles to break free, but can barely keep flying as she rapidly reaches orgasm and vanishes.");
				break;
			case fairymale:
				c.write(owner(),
						"The faeries zip through the air like a couple dogfighting planes. "
								+ opponent.own()
			...
			case impfem:
				c.write(owner(),
						own() + " faerie flies between the legs of " + opponent
								.own()
			...
			case slime:
				c.write(owner(),
						own() + "fae glows with magic as it circles " + opponent
								.own()
