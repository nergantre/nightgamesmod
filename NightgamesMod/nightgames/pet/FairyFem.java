...
public class FairyFem extends Pet {
                ...
                case 2:
                    if (!c.getStance().inserted(target)) {
                        if (target.crotchAvailable()) {
                            ...
                            target.body.pleasure(null, null, target.body.getRandom("cock"),
                                            2 + 3 * Global.random(power), c);
                        } else {
                            c.write(owner(), own() + "faerie slips into your "
                    ...
                    break;
                case 1:
                    c.write(owner(), own() + "faerie flies around " + owner().name()
                                    + ", channelling energy into her.");
                    owner().buildMojo(c, 20 + power);
                    break;
                default:
                    c.write(owner(), own() + "faerie rains magic energy on " + owner().name()
            ...
            switch (Global.random(4)) {
                case 3:
                    if (target.breastsAvailable()) {
                        c.write(owner(), "Your faerie lands on " + target.name()
                                        + "'s tit and plays with her sensitive nipple.");
                        target.body.pleasure(null, null, target.body.getRandom("breasts"), 3 + 2 * Global.random(power),
                                        c);
    ...
    public void vanquish(Combat c, Pet opponent) {
        switch (opponent.type()) {
            case fairyfem:
                c.write(owner(), "The two faeries circle around each other vying for the upper hand. " + own()
            ...
            case fairymale:
                ...
                break;
            case impfem:
                c.write(owner(), own() + " faerie flies between the legs of " + opponent.own()
                ...
                break;
            case impmale:
                c.write(owner(), opponent.own() + "imp grabs at " + own()
