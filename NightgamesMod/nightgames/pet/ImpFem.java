...
public class ImpFem extends Pet {
		...
		if (target.human()) {
			...
			case 3:
				if (target.crotchAvailable() && !c.getStance().inserted(target)) {
					c.write(owner(),
							own() + "imp grabs your dick and begins sucking it hungrily until you push her away.");
				...
				break;
			case 2:
				c.write(owner(), "While your attention is focused on " + owner().name()
			...
			default:
				if (!target.mostlyNude()) {
					...
					if (Global.random(25) > article.dc()
						...
						c.write(owner(), own() + "imp pulls on your " + article.getName()
			...
			case 1:
				c.write(owner(),
						"Your imp masturbates until her hand is coated in her own love juices, then flicks the fluid at "
								+ target.name() + "'s face. You see her shiver slightly as the "
	...
	public void vanquish(Combat c, Pet opponent) {
		...
		case fairymale:
			...
			break;
		case impfem:
			c.write(owner(), "The two female imps grapple with each other and " + opponent.own() + " imp throws "
					+ own() + "imp to the floor. " + opponent.own() + " imp approaches to press her "
					+ "advantage, but " + own() + "imp's tail suddenly thrusts into her pussy. As the tails fucks her, "
					+ own() + "imp collects some of her own wetness and forces the "
		...
		case impmale:
			...
			break;
		case slime:
			c.write(owner(), own() + "imp shoves both her hands into " + opponent.own()
