	...
	public void act(Combat c, Character target) {
		if (target.human()) {
			switch (Global.random(4)) {
				case 3:
					if (target.crotchAvailable()
						...
						c.write(owner(), own()
								+ "imps stands at the periphery of the fight, touching herself idly.");
					}
					break;
				case 2:
					c.write(owner(),
							"While your attention is focused on " + owner()
									.name()
							+ ", her imp creeps close to you and uppercuts you in the balls.");
				...
				case 1:
					if (c.getStance().prone(target)
						...
						c.write(owner(), own()
								+ "imp straddles your face, forcing her wet pussy onto your nose and mouth. Her scent is unnaturally intoxicating and fires up your libido.");
					} else {
						c.write(owner(), own()
				...
				default:
					if (!target.mostlyNude()) {
						...
						if (Global.random(25) > article.dc()
								+ (target.getStamina().percent()
										- target.getArousal().percent()) / 4
								|| !target.canAct()) {
							c.write(owner(), own() + "imp grabs your "
									+ article.getName() + " and yanks it off.");
							target.strip(article, c);
						} else {
							c.write(owner(),
									own() + "imp pulls on your " + article
											.getName()
			...
			switch (Global.random(3)) {
				case 2:
					c.write(owner(), "Your imp runs up to " + target.name()
							+ " and punches her in the gut.");
					target.pain(c, 2 + Global.random(power));
					break;
				case 1:
					c.write(owner(),
							"Your imp masturbates until her hand is coated in her own love juices, then flicks the fluid at "
									+ target.name()
									+ "'s face. You see her shiver slightly as the "
									+ "pheromone filled juices take effect.");
					target.arouse(2 + 2 * Global.random(power), c);
					break;
				default:
					if (!target.breastsAvailable()) {
						if (Global.random(25) > target.getOutfit()
								.getTopOfSlot(ClothingSlot.top).dc()
								+ (target.getStamina().percent()
										- target.getArousal().percent()) / 4
								|| !target.canAct()) {
							c.write(owner(),
									own() + "imp steals " + target.name()
											+ "'s "
											+ target.getOutfit()
													.getTopOfSlot(
															ClothingSlot.top)
						...
						if (Global.random(25) > target.getOutfit()
								.getTopOfSlot(ClothingSlot.bottom).dc()
								+ (target.getStamina().percent()
										- target.getArousal().percent()) / 4
								|| !target.canAct()) {
							c.write(owner(),
	...
	public void vanquish(Combat c, Pet opponent) {
		switch (opponent.type()) {
			case fairyfem:
				...
				break;
			case fairymale:
				c.write(owner(), own() + "imp catches " + opponent.own()
			...
			case impfem:
				c.write(owner(),
						"The two female imps grapple with each other and "
								+ opponent.own() + " imp throws " + own()
								+ "imp to the floor. " + opponent.own()
								+ " imp approaches to press her "
								+ "advantage, but " + own()
								+ "imp's tail suddenly thrusts into her pussy. As the tails fucks her, "
								+ own()
								+ "imp collects some of her own wetness and forces the "
								+ "aphrodisiac filled fluid into the other female's mouth. "
								+ opponent.own()
								+ "imp's orgasmic moan is stifled and she vanishes in a puff of brimstone.");
				break;
			case impmale:
				c.write(owner(), own() + "imp grapples with " + opponent.own()
			...
			case slime:
				c.write(owner(),
						own() + "imp shoves both her hands into " + opponent
								.own()
		...
		if (owner().human()) {
			c.write(captor,
					captor.name()
							...
							+ "to " + captor.name()
							+ " and makes a pleading sound. " + captor
									.name()
