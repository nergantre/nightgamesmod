...
public class FairyMale extends Pet {
			...
			switch (Global.random(4)) {
				case 3:
					if (target.crotchAvailable()) {
						c.write(owner(), "Your faerie flies between "
								+ target.name()
						...
						c.write(owner(),
								"Your faerie crawls into " + target.name()
										+ "'s "
										+ target.getOutfit().getTopOfSlot(
												ClothingSlot.bottom).getName()
				...
				case 1:
					if (Global.random(3) == 0) {
						c.write(owner(), "Your faerie flies behind "
								+ target.name()
								+ " and creates a small flash of light, distracting her.");
						target.add(c, new Flatfooted(target, 1));
					} else {
						c.write(owner(), "Your faerie flies at " + target.name()
								+ ", but she swats him away.");
					}
					break;
				default:
					c.write(owner(),
							"Your faerie lands on your shoulder and casts a spell, restoring your vitality.");
	...
	public void vanquish(Combat c, Pet opponent) {
		switch (opponent.type()) {
			case fairyfem:
				c.write(owner(),
						own() + "faerie boy chases " + opponent.own()
			...
			case impfem:
				c.write(owner(),
						own() + " faerie gets under " + opponent.own()
			...
			case slime:
				c.write(owner(),
						own() + " Glows as her surrounds himself with magic before charging at "
								+ opponent.own()
