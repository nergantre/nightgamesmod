...
public class Slime extends Pet {
	...
	public void act(Combat c, Character target) {
		...
		if (target.human()) {
			switch (Global.random(4)) {
				case 3:
					if (target.crotchAvailable()
						...
						target.body.pleasure(null, null,
								...
								2 + 3 * Global.random(power), c);
					} else {
						c.write(owner(), "You see eyes form in " + own()
								+ " slime as it watches the fight curiously.");
					}
					break;
				case 2:
					if (!target.getOutfit()
							.slotUnshreddable(ClothingSlot.top)) {
						c.write(owner(),
								own() + "slime forms into a shape that's vaguely human and clearly female. Somehow it manages to look cute and innocent while still being an animated blob of slime. "
										+ "While you're processing this, the slime jumps on you and your "
										+ target.getOutfit()
				...
				case 1:
					if (!c.getStance().prone(target)) {
						if (power * Global.random(20) >= target.knockdownDC()) {
							c.write(owner(), own()
									+ "slime wraps around your ankles and you are unable to keep your footing.");
							target.add(c, new Falling(target));
						} else {
							c.write(owner(), own()
									+ "slime glomps onto your ankles. You almost lose your balance, but manage to recover.");
						}
					} else {
						c.write(owner(), "You see eyes form in " + own()
								+ "slime as it watches the fight curiously.");
				...
				default:
					c.write(owner(), own()
							+ "slime takes on a humanoid shape and watches you like a curious child.");
			}
		} else {
			switch (Global.random(4)) {
				case 3:
					if (target.crotchAvailable()
						...
						c.write(owner(),
								"Two long appendages extend from your slime and wrap around "
										+ target.name()
				...
				case 2:
					if (!target.getOutfit()
							.slotUnshreddable(ClothingSlot.top)) {
						c.write(owner(),
								"Your slime pounces on " + target.name()
										+ " playfully, and it's corrosive body melts her "
										+ target.getOutfit()
												.getTopOfSlot(ClothingSlot.top)
												.getName()
					...
					if (!target.getOutfit()
						...
						c.write(owner(), "You slime hugs " + target.name()
								+ " affectionately, covering her in slimy liquid.");
						target.add(c, new Oiled(target));
					}
					break;
				case 1:
					if (!c.getStance().prone(target)) {
						if (power * Global.random(20) >= target.knockdownDC()) {
							c.write(owner(), target.name()
									+ " slips on your slime as it clings to her feet, losing her balance.");
							target.add(c, new Falling(target));
						} else {
							c.write(owner(), target.name()
									+ " stumbles as your slime clings to her leg. She manages to catch herself and scrapes off the clingly blob.");
						}
					} else {
						c.write(owner(), "You see eyes form in " + own()
								+ "slime as it watches the fight curiously.");
				...
				default:
					c.write(owner(),
							"Your slime forms into a girlish shape and looks up at you, as if seeking approval.");
			}
		}
	}
	...
	public void vanquish(Combat c, Pet opponent) {
		switch (opponent.type()) {
			case fairyfem:
				c.write(owner(), opponent.own() + "faerie flies over " + own()
			...
			case fairymale:
				c.write(owner(),
						opponent.own() + "faerie flies too close to " + own()
			...
			case impfem:
				c.write(owner(),
						own() + "slime gathers around " + opponent.own()
				...
				break;
			case impmale:
				c.write(owner(), opponent.own() + "imp grabs for " + own()
			...
			case slime:
				c.write(owner(),
						"The two slimes circle around each other, while gradually taking on human shape. One of the oozes looks vaguely like "
								+ own()
								+ "small slimy twin, while the other takes "
								+ opponent.own()
