...
public class Slime extends Pet {
		...
		if (target.human()) {
			...
			case 1:
				if (!c.getStance().prone(target)) {
					if (power * Global.random(20) >= target.knockdownDC()) {
						c.write(owner(),
								own() + "slime wraps around your ankles and you are unable to keep your footing.");
			...
			case 3:
				if (target.crotchAvailable() && !c.getStance().vaginallyPenetrated(target)) {
					c.write(owner(),
							"Two long appendages extend from your slime and wrap around " + target.name()
			...
			case 2:
				if (!target.getOutfit().slotUnshreddable(ClothingSlot.top)) {
					c.write(owner(),
							"Your slime pounces on " + target.name() + " playfully, and it's corrosive body melts her "
									+ target.getOutfit().getTopOfSlot(ClothingSlot.top).getName()
				...
				if (!target.getOutfit().slotUnshreddable(ClothingSlot.bottom)) {
					...
					c.write(owner(),
							"You slime hugs " + target.name() + " affectionately, covering her in slimy liquid.");
					target.add(c, new Oiled(target));
				}
				break;
			case 1:
				if (!c.getStance().prone(target)) {
					if (power * Global.random(20) >= target.knockdownDC()) {
						c.write(owner(),
								target.name() + " slips on your slime as it clings to her feet, losing her balance.");
						target.add(c, new Falling(target));
					} else {
	...
	public void vanquish(Combat c, Pet opponent) {
		...
		case impmale:
			c.write(owner(),
					opponent.own() + "imp grabs for " + own()
