...
public enum Trait {
	...
	vaginaltongue("Vaginal Tongue", "Have a second tongue", (b, c, t) -> {
		if (c.crotchAvailable()) {
			b.append(
					"Occasionally, a pink tongue slides out of her pussy and licks her lower lips.");
		}
	}),

	// Perks
	smqueen("SM Queen",
			"Skilled at providing pleasure alongside pain", (b, c,
					t) -> b.append(Global.capitalizeFirstLetter(String.format(
							"%s sneers in a way like an SM queen.",
							c.subject())))),
	ticklemonster("Tickle Monster",
			"Skilled at tickling in unconventional areas"), // Mara Sex perk,
															// increases
															// pleasure from
															// tickling if
															// target is nude
	...
	desensitized("Desensitized", "Sex is old hat now"), // Reyka Sex perk slight
														// pleasure reduction
	desensitized2("Desensitized 2",
			"Only the strongest stimulation gets you off"), // Reyka Sex perk
															// slight pleasure
															// reduction
	RawSexuality("Raw Sexuality",
			"Constant lust boost for you and your opponent in battle",
			(b, c, t) -> {
				if (c.human()) {
					b.append("You exude");
				} else {
				...
				b.append(
						" an aura of pure eros, making both of you flush with excitement.");
			}), // Eve
	...
	augmentedPheromones("Augmented Pheromones",
			"Artificially enhanced pheromones", null, pheromones),
	...
	lacedjuices("Laced Juices", "Intoxicating bodily fluids"), // opponents take
																// temptation
																// when using
																// oral skills
	...
	lactating("Lactating", "Breasts produces milk", (b, c, t) -> {
		if (!c.human()) {
			if (c.breastsAvailable()) {
				b.append(
						"You occasionally see milk dribbling down her breasts. Is she lactating?");
			} else {
			...
			b.append(
					"Your nipples ache from the milk building up in your mammaries.");
		}
	}),
	...
	objectOfWorship("Object Of Worship",
			"Opponents is periodically forced to worship your body.",
			(b, c, t) -> b.append(
					"A divine aura surrounds " + c.nameDirectObject() + ".")),
	spiritphage("Semenphage", "Feeds on semen"),
	erophage("Erophage", "Feeds on sexuality"),
	tight("Tight",
			"Powerful musculature and exquisite tightness makes for quick orgasms."),
	holecontrol("Pussy Control", "Dexterous internal muscle control."),
	oiledass("Oiled Ass", "Natural oils makes her ass always ready."),
	autonomousAss("Autonomous Ass",
			"Asshole instinctively forces anything inside of it to cum."),
	fetishTrainer("Fetish Trainer", "Capable of developing others' fetishes."),
	insertion("Insertion Master", "More pleasure on insertion"), // more damage
																	// on
																	// insertion.
	proheels("Heels Pro", "Pro at walking around in heels"), // no speed penalty
																// from heels
	masterheels("Heels Master",
			"Master at moving in heels, resists knockdowns"), // graceful when
																// wearing heels
	...
	freeSpirit("Free Spirit",
			"Better at escaping pins and resisting mind control"),
	...
	autonomousPussy("Autonomous Pussy",
			"Her pussy instinctively forces anything inside of it to cum."),
	// AI traits
	...
	imagination("Active Imagination", "More easily swayed by pillow talk"), // more
																			...
																			// indirect
																			// skills
	achilles("Achilles Jewels",
			"Delicate parts are somehow even more delicate"), // more pain from
																// groin attacks
	...
	lethargic("Lethargic", "Very low mojo gain from normal methods.",
			new Lethargic(null, 999, .75)), // 25% mojo gain
	...
	// Restrictions
	softheart("Soft Hearted", "Incapable of being mean"), // restricts slap,
															// stomp, flick
	...
	demigoddess("Demigoddess",
			"Blessed by a deity of sexual pleasure, and on the road to ascension herself."),
	fighter("Fighter", "A combination of martial arts and ki"),
	slime("Slime",
			"An accident in the biology labs made the body a bit more... malleable."),
	...
	experienced("Experienced Lover",
			"Skilled at pacing yourself when thrusting"), // reduced recoil from
															// penetration
	...
	PersonalInertia("Personal Inertia",
			"status effects (positive and negative) last 50% longer"),
	Confident("Confident", "Mojo decays slower out of combat"),
	SexualGroove("Sexual Groove", "Passive mojo gain every turn in combat"),
	BoundlessEnergy("Boundless Energy",
			"Increased passive stamina gain in battle"),
	...
	sympathetic("Sympathetic",
			"Intervening opponents are more likely to side with you"),
	...
	strapped("Strapped", "Penis envy", (b, c, t) -> {
		if (c.human()) {
			...
			b.append("A large black strap-on dildo adorns " + c.name()
					+ "'s waists.");
		}
	...
	static {
		...
		resistances.put(shameless, (c, s) -> {
			if (s.flags().contains(Stsflag.shamed)
					|| s.flags().contains(Stsflag.distracted)) {
				return "Shameless";
			}
			return "";
		});
		resistances.put(Trait.freeSpirit, (c, s) -> {
			...
			if ((s.flags().contains(Stsflag.enthralled)
					|| s.flags().contains(Stsflag.bound))
		...
		resistances.put(Trait.calm, (c, s) -> {
			...
			if ((s.flags().contains(Stsflag.horny)
					|| s.flags().contains(Stsflag.hypersensitive))
		...
		resistances.put(Trait.masterheels, (c, s) -> {
			...
			if (c.has(ClothingTrait.heels)
					&& s.flags().contains(Stsflag.falling)
					&& Global.random(100) < 33) {
				return "Heels Master";
			}
			return "";
		});
