...
public enum CockMod implements BodyPartMod {
	...
	public double applyBonuses(Character self, Character opponent,
			BodyPart target, double damage, Combat c, ModdedCockPart part) {
		double bonus = part.getBase().applyBonuses(self, opponent, target,
				damage, c);
		...
		if (this == runic) {
			...
			if (Global.random(8) == 0 && !opponent.wary()) {
				message += String.format(
						"Power radiates out from %s %s, seeping into %s and subverting %s will. ",
						self.nameOrPossessivePronoun(), part.describe(self),
						opponent.nameOrPossessivePronoun(),
			...
			if (self.hasStatus(Stsflag.cockbound)) {
				String binding = ((CockBound) self
						.getStatus(Stsflag.cockbound)).binding;
				message += String.format(
			...
			String message = String.format(
					"%s demonic appendage latches onto %s will, trying to draw it into %s.",
			...
			if (target == PussyPart.feral) {
				message += String.format(
						" %s %s gladly gives it up, eager for more pleasure.",
						opponent.possessivePronoun(),
						target.describe(opponent));
				amtDrained = 5;
				bonus += 2;
				...
				message += String.format(
						" Despite %s best efforts, some of the elusive energy passes into %s.",
						opponent.nameOrPossessivePronoun(),
						self.nameDirectObject());
				amtDrained = 3;
			}
			if (amtDrained != 0) {
			...
			if (Global.random(5) == 0 && target.getType().equals("pussy")) {
				message += String.format(
						...
						part.describe(self), opponent.possessivePronoun(),
			...
			if (target == PussyPart.succubus) {
				message = String.format(
						...
						self.subjectAction("focus", "focuses"),
						self.possessivePronoun(), self.possessivePronoun(),
						part.describe(self));
				c.write(self, message);
				// Actual bad effects are dealt with in PussyPart
			} else {
				message = String
						.format("Drawing upon %s extensive training, %s %s will into %s %s, enhancing %s own abilities",
								self.possessivePronoun(),
								self.subjectAction("concentrate",
										"concentrates"),
						self.possessivePronoun(), self.possessivePronoun(),
						part.describe(self), self.possessivePronoun());
				c.write(self, message);
				for (int i = 0; i < Math.max(2,
						(self.get(Attribute.Ki) + 5) / 10); i++) { // +5
																	// for
																	// rounding:
																	// 24->29->20,
																	// 25->30->30
					Attribute attr = new Attribute[] { Attribute.Power,
							Attribute.Cunning, Attribute.Seduction }[Global
									.random(3)];
	...
	public double applySubBonuses(Character self, Character opponent,
		...
		return moddedCockPart.getBase().applySubBonuses(self, opponent, with,
				target, damage, c);
	}
	...
	public double applyReceiveBonuses(Character self, Character opponent,
				...
				c.write(self,
						Global.format(
								"{self:NAME-POSSESSIVE} "
										+ moddedCockPart.fullDescribe(self)
		...
		return moddedCockPart.getBase().applyReceiveBonuses(self, opponent,
				target, damage, c);
	}
	...
	public void onOrgasm(Combat c, Character self, Character opponent,
			BodyPart target, boolean selfCame, CockPart part) {
		if (this == incubus && c.getStance().inserted(self)) {
			if (selfCame) {
				...
				if (target != PussyPart.cybernetic) {
					...
					opponent.add(c, new Abuff(opponent, Attribute.Power,
							-attDamage, 20));
					opponent.add(c, new Abuff(opponent, Attribute.Cunning,
							-attDamage, 20));
					opponent.add(c, new Abuff(opponent, Attribute.Seduction,
							-attDamage, 20));
					self.add(c,
							new Abuff(self, Attribute.Power, attDamage, 20));
					self.add(c,
							new Abuff(self, Attribute.Cunning, attDamage, 20));
					self.add(c, new Abuff(self, Attribute.Seduction, attDamage,
							20));
	...
	public void tickHolding(Combat c, Character self, Character opponent,
			BodyPart otherOrgan, CockPart part) {
		if (this == primal) {
			c.write(self,
					String.format(
							"Raw sexual energy flows from %s %s into %s %s, enflaming %s lust",
							self.nameOrPossessivePronoun(), part.describe(self),
							opponent.nameOrPossessivePronoun(),
							otherOrgan.describe(opponent),
							opponent.possessivePronoun()));
	...
	public void onStartPenetration(Combat c, Character self, Character opponent,
			BodyPart target, ModdedCockPart moddedCockPart) {
