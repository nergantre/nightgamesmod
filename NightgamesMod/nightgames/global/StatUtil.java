...
public class StatUtil {
	...
	public static double getInvCDF(double d, boolean highPrecision) {
		...
		if (d == 0) {
			...
			z = (((((ICDF_C[0] * q + ICDF_C[1]) * q + ICDF_C[2]) * q
					+ ICDF_C[3]) * q + ICDF_C[4]) * q + ICDF_C[5])
					/ ((((ICDF_D[0] * q + ICDF_D[1]) * q + ICDF_D[2]) * q
							+ ICDF_D[3]) * q + 1);
		}
		...
		// Rational approximation for central region:
			...
			double q = d - 0.5D;
			double r = q * q;
			z = (((((ICDF_A[0] * r + ICDF_A[1]) * r + ICDF_A[2]) * r
					+ ICDF_A[3]) * r + ICDF_A[4]) * r + ICDF_A[5]) * q
					/ (((((ICDF_B[0] * r + ICDF_B[1]) * r + ICDF_B[2]) * r
							+ ICDF_B[3]) * r + ICDF_B[4]) * r + 1);
		}
		if (highPrecision) {
			z = refine(z, d);
		}
	...
	private static final double	X_HUGE		= 1.0D
			/ (2.0D * Math.sqrt(X_SMALL));
