...
public class Challenge implements Deployable {
	...
	public GOAL pick() {
		...
		if (target.has(Item.Strapon) || target.has(Item.Strapon2)
				|| target.hasDick()) {
		...
		switch (goal) {
			...
			case clothedwin:
				return "Not everyone relies on brute force to get their opponents off. The masters of seduction often don't bother to even undress their opponents. See "
						+ "if you can make " + target.name()
						+ " cum while she's still got her clothes on.";
			case bathambush:
				return "";
			case peggedloss:
				return "Getting pegged in the ass is a hell of a thing, isn't it. I sympathize... especially since "
						+ target.name()
						+ " seems to have it in for you tonight. If it "
			...
			case pendomwin:
				return "How good are you exactly? If you want to show "
						+ target.name()
			...
			case subwin:
				return "Everyone loves an underdog. If you can win a fight with "
						+ target.name()
	...
	public void check(Combat state, Character victor) {
		if (!done && (state.p1 == target || state.p2 == target
			...
			switch (goal) {
				case kisswin:
					if (victor == owner
							&& state.lastact(owner).toString().equals("Kiss")) {
						done = true;
					}
					break;
				case clothedwin:
					if (victor == owner && !target.breastsAvailable()
	...
	public void resolve(Character active) {
		if (active.state == State.ready) {
			owner = active;
			target = Global.getMatch().combatants.get(
					Global.random(Global.getMatch().combatants.size() - 1));
