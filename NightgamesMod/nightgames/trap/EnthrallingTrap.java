...
public class EnthrallingTrap implements Trap {
	...
	public void trigger(Character target) {
		if (target.human()) {
			if (target.check(Attribute.Perception, 25 - target.get(Attribute.Perception) + target.baseDisarm())
				...
				Global.gui()
						.message("As you step across the " + target.location().name
			...
			if (target.location().humanPresent()) {
				Global.gui()
						.message("You catch a bout of purple fire in your peripheral vision,"
	...
	public String setup(Character owner) {
		...
		return "You pop open a bottle of cum and use its contents to draw"
				...
				+ " incantations to cause the first person to step into" + " it to be immediatly enthralled by you.";
	}
