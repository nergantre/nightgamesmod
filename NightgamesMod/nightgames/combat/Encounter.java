	...
	public boolean spotCheck() {
		if (p1.eligible(p2) && p2.eligible(p1)) {
			...
			if (p1.state == State.masturbating) {
				...
				Global.gui().message(
						"You encounter " + p2.name() + ", but you still haven't recovered from your last fight.");
			} else if (p1.human()) {
			...
			if (p1ff && p2ff) {
				...
				if (p2.check(Attribute.Speed, 10 + p1.get(Attribute.Speed) + (p1.has(Trait.sprinter) ? 5 : 0)
					...
					if (p1.human() || p2.human()) {
						if (p1.human()) {
							Global.gui().message(
									p2.name() + " tries to run, but you stay right on her heels and catch her.");
						} else {
				...
				if (p1.check(Attribute.Speed, 10 + p2.get(Attribute.Speed) + (p1.has(Trait.sprinter) ? -5 : 0)
					...
					if (p1.human() || p2.human()) {
						if (p2.human()) {
							Global.gui().message(
									p1.name() + " tries to run, but you stay right on her heels and catch her.");
						} else {
		...
		if (target.human()) {
			Global.gui().message(
					"You jerk off frantically, trying to finish as fast as possible. Just as you feel the familiar sensation of imminent orgasm, you're grabbed from behind. "
							...
							+ attacker.name() + " kisses you on the lips and rubs the head of your penis with her "
