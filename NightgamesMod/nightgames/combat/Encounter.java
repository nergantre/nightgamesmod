...
public class Encounter implements Serializable, IEncounter {
	...
	public boolean spotCheck() {
		if (p1.eligible(p2) && p2.eligible(p1)) {
			if (p1.state == State.shower) {
				...
				spider(p1, p2);
			} else if (p1.state == State.crafting
					|| p1.state == State.searching) {
				p2.spy(p1, this);
				return true;
			} else if (p2.state == State.crafting
					|| p2.state == State.searching) {
				p1.spy(p2, this);
				return true;
			}
			if (p1.state == State.shower) {
	...
	private void fightOrFlight(Character p, boolean fight) {
		...
		if (checkin >= 2) {
			if (p1ff && p2ff) {
				...
				if (p2.check(Attribute.Speed,
						10 + p1.get(Attribute.Speed)
								+ (p1.has(Trait.sprinter) ? 5 : 0)
								+ (p2.has(Trait.sprinter) ? -5 : 0))) {
					if (p1.human()) {
						Global.gui().message(p2.name()
								+ " dashes away before you can move.");
					}
					p2.flee(location);
				} else {
					fightTime = 2;
					if (p1.human() || p2.human()) {
						if (p1.human()) {
							...
							Global.gui()
									.message("You quickly try to escape, but "
											+ p1.name()
				...
				if (p1.check(Attribute.Speed,
						10 + p2.get(Attribute.Speed)
								+ (p1.has(Trait.sprinter) ? -5 : 0)
								+ (p2.has(Trait.sprinter) ? 5 : 0))) {
					if (p2.human()) {
						Global.gui().message(p1.name()
				...
				if (p1.get(Attribute.Speed)
						+ Global.random(10) >= p2.get(Attribute.Speed)
								+ Global.random(10)) {
					if (p2.human()) {
						Global.gui().message(p1.name()
								+ " dashes away before you can move.");
	...
	private void ambush(Character attacker, Character target) {
		...
		if (p1.human() || p2.human()) {
			...
			if (target.human()) {
				Global.gui().message(attacker.name()
						+ " catches you by surprise and attacks!");
			}
		} else {
			// this.fight=new NullGUI().beginCombat(p1,p2);
			Global.gui().refresh();
	...
	private void showerambush(Character attacker, Character target) {
		...
		if (target.human()) {
			if (location.id() == Movement.shower) {
				...
				Global.gui().message(
						"The relaxing water causes you to lower your guard a bit, so you don't notice "
								+ attacker.name()
								+ " until she's standing over you. There's no chance to escape, you'll have to face her nude.");
			}
		} else if (attacker.human()) {
			if (location.id() == Movement.shower) {
				Global.gui().message("You stealthily walk up behind "
						+ target.name()
				...
				Global.gui().message("You creep up to the jacuzzi where "
						+ target.name()
	...
	private void aphrodisiactrick(Character attacker, Character target) {
		...
		if (target.human()) {
			...
			if (location.id() == Movement.shower) {
				...
				Global.gui().message(
						"You sneak up to the jacuzzi, and empty the aphrodisiac into the water without "
								+ target.name()
								+ " noticing. You slip away and find a hiding spot. In a "
	...
	private void spider(Character attacker, Character target) {
		...
		if (attacker.human()) {
			...
			Global.gui().message(
					"You're trying to figure out a way to free yourself, when you see "
							+ attacker.name()
	...
	public void trap(Character opportunist, Character target, Trap trap) {
		if (opportunist.human()) {
			Global.gui().message("You leap out of cover and catch "
					+ target.name() + " by surprise.");
		} else if (target.human()) {
			Global.gui().message("Before you have a chance to recover, "
					+ opportunist.name() + " pounces on you.");
		}
		Global.gui().refresh();
