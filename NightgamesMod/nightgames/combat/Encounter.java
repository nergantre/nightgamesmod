...
public class Encounter implements Serializable, IEncounter {
    ...
    protected void checkEnthrall(Character p1, Character p2) {
        ...
        if (enthrall != null) {
            if (((Enthralled) enthrall).master != p2) {
                ...
                if (p1.human()) {
                    Global.gui().message("At " + p2.name() + "'s interruption, you break free from the"
                                    + " succubus' hold on your mind. However, the shock all but"
                                    + " short-circuits your brain; you "
                                    + " collapse to the floor, feeling helpless and" + " strangely oversensitive");
                } else if (p2.human()) {
                    Global.gui().message(p1.name() + " doesn't appear to notice you at first, but when you "
    ...
    public boolean spotCheck() {
        if (p1.eligible(p2) && p2.eligible(p1)) {
            ...
            if (p1.state == State.masturbating) {
                if (p1.human()) {
                    Global.gui().message(
                                    p2.name() + " catches you masturbating, but fortunately she's still not allowed to attack you, so she just watches you jerk off with "
                                                    + "an amused grin.");
                } else if (p2.human()) {
                    Global.gui().message("You stumble onto " + p1.name()
                ...
                if (p2.human()) {
                    Global.gui().message(
                                    p1.name() + " catches you masturbating, but fortunately she's still not allowed to attack you, so she just watches you jerk off with "
                                                    + "an amused grin.");
                } else if (p1.human()) {
                    Global.gui().message("You stumble onto " + p2.name()
                                    + " with her hand between her legs, masturbating. Since you just fought, you still can't touch her, so "
                                    + "you just watch the show until she orgasms.");
                }
            } else if (!p1.eligible(p2) && p1.human()) {
                Global.gui().message("You encounter " + p2.name()
                                + ", but you still haven't recovered from your last fight.");
            } else if (p1.human()) {
                Global.gui().message("You find " + p2.name()
    ...
    protected void fightOrFlight(Character p, boolean fight) {
        ...
        if (checkin >= 2) {
            if (p1ff && p2ff) {
                ...
                if (p2.check(Attribute.Speed, 10 + p1.get(Attribute.Speed) + (p1.has(Trait.sprinter) ? 5 : 0)
                    ...
                    if (p1.human() || p2.human()) {
                        if (p1.human()) {
                            Global.gui().message(p2.name()
                                            + " tries to run, but you stay right on her heels and catch her.");
                        } else {
                            Global.gui().message("You quickly try to escape, but " + p1.name()
                ...
                if (p1.check(Attribute.Speed, 10 + p2.get(Attribute.Speed) + (p1.has(Trait.sprinter) ? -5 : 0)
                                + (p2.has(Trait.sprinter) ? 5 : 0))) {
                    if (p2.human()) {
                    ...
                    if (p1.human() || p2.human()) {
                        if (p2.human()) {
                            ...
                            Global.gui().message("You quickly try to escape, but " + p2.name()
                                            + " is quicker. She corners you and attacks.");
                        }
                        this.fight = Global.gui().beginCombat(p1, p2);
                    } else {
    ...
    protected void showerambush(Character attacker, Character target) {
        ...
        if (target.human()) {
            if (location.id() == Movement.shower) {
                Global.gui().message(
                                "You aren't in the shower long before you realize you're not alone. Before you can turn around, a soft hand grabs your exposed penis. "
                                                + attacker.name() + " has the drop on you.");
            } else if (location.id() == Movement.pool) {
                Global.gui().message("The relaxing water causes you to lower your guard a bit, so you don't notice "
                                + attacker.name()
                                + " until she's standing over you. There's no chance to escape, you'll have to face her nude.");
            }
        } else if (attacker.human()) {
            if (location.id() == Movement.shower) {
                Global.gui().message("You stealthily walk up behind " + target.name()
                ...
                Global.gui().message("You creep up to the jacuzzi where " + target.name()
    ...
    protected void aphrodisiactrick(Character attacker, Character target) {
        ...
        if (target.human()) {
            if (location.id() == Movement.shower) {
                Global.gui().message(
                                "The hot shower takes your fatigue away, but you can't seem to calm down. Your cock is almost painfully hard. You need to deal with this while "
                ...
                Global.gui().message(
                                "As you relax in the jacuzzi, you start to feel extremely horny. Your cock is in your hand before you're even aware of it. You stroke yourself "
            ...
            if (location.id() == Movement.shower) {
                Global.gui().message(
                                "You empty the bottle of aphrodisiac onto the shower floor, letting the heat from the shower turn it to steam. You watch "
                ...
                Global.gui().message("You sneak up to the jacuzzi, and empty the aphrodisiac into the water without "
    ...
    protected void caught(Character attacker, Character target) {
        ...
        if (target.human()) {
            Global.gui().message(
                            "You jerk off frantically, trying to finish as fast as possible. Just as you feel the familiar sensation of imminent orgasm, you're grabbed from behind. "
            ...
            if (!target.mostlyNude()) {
                Global.gui().message(
                                "You groan in resignation and reluctantly strip off your clothes and hand them over.");
            }
        } else if (attacker.human()) {
            Global.gui().message("You spot " + target.name()
    ...
    protected void spider(Character attacker, Character target) {
        ...
        if (attacker.human()) {
            Global.gui().message(
                            target.name() + " is naked and helpless in the giant rope web. You approach slowly, taking in the lovely view of her body. You trail your fingers "
