...
public class Combat extends Observable implements Serializable, Cloneable{
	...
	public void turn(){
		...
		message = p2.describe(p1.get(Attribute.Perception))+"<p>"+getStance().describe()+"<p>"+p1.describe(p2.get(Attribute.Perception))+"<p>";
		phase=1;
		p1.regen(this);
		p2.regen(this);
		p1act=null;
	...
	public void act(Character c, Skill action){
		...
		if(p1act!=null&&p2act!=null){
			...
			if(p1.init()+p1act.speed()>=p2.init()+p2act.speed()){
				if(Skill.skillIsUsable(this, p1act, p2)){
					p1act.resolve(this,p2);
					checkStamina(p2);
				}
				write("<br>");
				if(Skill.skillIsUsable(this, p2act, p1)){
					p2act.resolve(this,p1);
					checkStamina(p1);
				}
			}
			else{
