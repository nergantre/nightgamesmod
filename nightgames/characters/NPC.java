...
public class NPC extends Character {
	...
	public String describe(int per, Combat c) {
		...
		if (top.empty() && bottom.empty()) {
			...
			if (top.empty()) {
				...
				description = description + "She is wearing " + top.peek().pre()
						+ top.peek().getName() + " and ";
			}
			if (bottom.empty()) {
				description = description + "is naked from the waist down.<br>";
			} else {
				description = description + bottom.peek().pre()
						+ bottom.peek().getName() + ".<br>";
			}
		}
	...
	public void victory(Combat c, Result flag) {
		...
		dress(c);
		target.undress(c);
		if (target.outfit[1].isEmpty()
				|| c.clothespile.contains(target.outfit[1].firstElement())) {
	...
	public void defeat(Combat c, Result flag) {
		...
		target.dress(c);
		this.undress(c);
		if (this.outfit[1].isEmpty()
				|| c.clothespile.contains(this.outfit[1].firstElement())) {
		...
		switch (type) {
			...
			case fucking:
				if (c.getStance().sub(this)
						&& c.getStance().reverse() != c.getStance()) {
					if (c.getStance().inserted(this)) {
						c.write(this,
								Global.format(
										"{self:SUBJECT-ACTION:pinch|pinches} {other:possessive} clitoris with {self:possessive} hands as {other:subject-action:try|tries} to trust into {self:direct-object}. "
												+ "While {other:subject-action:yelp|yelps} with surprise, {self:subject-action:rotate|rotates} {self:possessive} body around into a dominant position",
										this, target));
					} else if (target.hasBalls()) {
						c.write(this,
								Global.format(
										"{self:SUBJECT-ACTION:squeezes|squeezes} {other:possessive} balls with {self:possessive} hands as {other:subject-action:try|tries} to ride {self:direct-object}. "
												+ "While {other:subject-action:yelp|yelps} with surprise, {self:subject-action:rotate|rotates} {self:possessive} body around into a dominant position",
										this, target));
					} else {
						c.write(this,
								Global.format(
										"{self:SUBJECT-ACTION:pinch|pinches} {other:possessive} nipples with {self:possessive} hands as {other:subject-action:try|tries} to ride {self:direct-object}. "
												+ "While {other:subject-action:yelp|yelps} with surprise, {self:subject-action:rotate|rotates} {self:possessive} body around into a dominant position",
										this, target));
					}
					...
					if (c.getStance().inserted(this)) {
						target.body
								.pleasure(this, body.getRandomInsertable(),
										target.body.getRandomHole(),
										4 + Math.min(
												Global.random(
														get(Attribute.Seduction)),
												20),
						...
						target.body
								.pleasure(this, body.getRandomHole(),
	...
	public void eot(Combat c, Character opponent, Skill last) {
		...
		if (opponent.has(Trait.pheromones)
			...
			add(c, new Horny(this,
					opponent.has(Trait.augmentedPheromones) ? 2 : 1, 10,
	...
	private float rateMove(Skill skill, Combat c, float urg, float fit1,
			float fit2) {
