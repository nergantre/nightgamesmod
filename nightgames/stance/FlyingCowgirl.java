...
public class FlyingCowgirl extends FemdomSexStance {
	...
	public Position reverse(Combat c) {
		if (bottom.body.getRandomWings() != null) {
			c.write(bottom, Global.format("In a desperate gamble for dominance, {self:subject} piston wildly into {other:name-do}, making {other:direct-object} yelp and breaking {other:possessive} concentration. Shaking off {other:possessive} limbs coiled around {self:subject}, {self:subject} starts flying on {self:possessive} own and starts fucking {other:direct-object} back in the air.", bottom, top));
