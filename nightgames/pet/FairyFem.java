...
public class FairyFem extends Pet {
	...
	public void act(Combat c, Character target) {
		if(target.human()){
			...
			case 2:
				...
				break;
			case 1:
				c.write(owner(),"Your faerie circles around your with a faint glow and kisses you on the cheek. You feel energy building inside you.");
				owner().buildMojo(c, 20);
				break;
			default:
				c.write(owner(),"Your faerie flies next to your ear in speaks to you. The words aren't in english and you have no idea what she said, but somehow you feel your fatigue drain away.");
				owner().heal(c, power+Global.random(10));
			}
		}
	}
	...
	public void vanquish(Combat c, Pet opponent) {
		...
		case fairyfem:
			c.write(owner(),"The two faeries circle around each other vying for the upper hand. "+own()+"faerie catches "+opponent.own()+"faerie by the hips and starts to eat her out. " +
					opponent.own()+" fae struggles to break free, but can barely keep flying as she rapidly reaches orgasm and vanishes.");
			break;
		case fairymale:
			c.write(owner(),"The faeries zip through the air like a couple dogfighting planes. "+opponent.own()+"male manages to catch the female's hands, but you see her foot shoot up decisively " +
					"between his legs. The stricken male tumbles lazily to the floor and vanishes in midair.");
			break;
		case impfem:
			c.write(owner(),own()+" faerie flies between the legs of "+opponent.own()+"imp, slipping both arms into the larger pussy. The imp trembles as falls to the floor as the faerie puts her entire " +
					"upper body into pleasuring her. "+own()+"faerie is forcefully expelled by the imp's orgasm just before the imp vanishes.");
			break;
		case impmale:
			c.write(owner(),opponent.own()+"imp grabs at "+own()+"faerie, but the nimble sprite changes direct in midair and darts between the imp's legs. You can't see exactly what happens next, " +
					"but the imp clutches his groin in pain and disappears.");
			break;
		case slime:
			c.write(owner(),own()+"fae glows with magic as it circles "+opponent.own()+"slime rapidly. The slime begins to tremble and slowly elongates into the shape of a crude phallis. It shudders " +
	...
	public void caught(Combat c, Character captor) {
		if(captor.human()){
			...
			c.write(captor,captor.name()+" manages to catch your faerie and starts pleasuring her with the tip of her finger. The sensitive fae clings to the probing finger desperately as she thrashes " +
