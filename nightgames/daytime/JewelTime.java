...
public class JewelTime extends Activity {
	...
	public void visit(String choice) {
		...
		if(choice == "Start"){
			if(jewel.getAffection(player)>25&&jewel.has(Trait.fighter)){
			...
			else if(jewel.getAttraction(player)<15){
				Global.gui().message("You wait by the campus gardens, which according to Aesop, is on Jewel's jogging route. Sure enough, she comes running by within ten minutes and " +
					"calls out to you when she notices you. <i>\"Hey "+player.name()+". It's good to see you. Are you here to train too?\"</i> She's not breathing particularly heavily and there's only " +
					"a subtle touch of sweat on her skin, so presumeably you've caught her early in her run. You could pretend to have run into her by coincidence, but " +
					...
					"exercise clothes accentuate her toned body, especially when she quickens her pace and pulls ahead of you. She catches you oogling her and smiles. <i>\"Do you like what you see? " +
				...
				jewel.gainAttraction(player,2);
				player.gainAttraction(jewel,2);
			}
			else{
				Global.gui().message("You look around the campus and manage to catch Jewel while she's preparing for her run. She's doing stretches in a tight workout outfit that shows " +
				...
				jewel.gainAffection(player,1);
				player.gainAffection(jewel,1);
				Global.gui().choose(this,"Games");
				Global.gui().choose(this,"Sparring");
				Global.gui().choose(this,"Sex");
			}
			...
			if(jewel.getAffection(player)>=16&&(!player.has(Trait.spiral)||Global.random(2)==1)){
				Global.gui().message("Jewel leads you to her bedroom instead of the wilderness for once. She seems unusually eager and almost like she could break into skipping at any time. " +
						...
						"still breathing heavily, but she gives you one of her most agressive grins. <i>\"That was a good start, but I've never been a one orgasm girl. Besides, as long as you've " +
						...
						"can do this. I believe in you.\"</i> There's no way you can disappoint her after coming this far. You put everything you have left into thrusting your hips, determined to make her " +
				...
				if(!player.has(Trait.spiral)){
					Global.gui().message("<p><b>You've learned to harness the awesome power that has laid dormant in your soul... maybe. You've at least learned to move your hips a lot during sex.</b>");
					player.add(Trait.spiral);
					jewel.add(Trait.spiral);
				}
			}
			else{
				Global.gui().message("<i>\"Come on, it's not far now.\"</i> Jewel encourages you as you fall behind her slightly. The two of you are running at a slower pace than usual. You " +
						...
						"up and turns to you. <i>\"This is great, but we came here with some very important activities planned. Shall we?\"</i> When you stand up, she lets out an uncharactaristically " +
			...
			Global.gui().choose(this,"Leave");
			Daytime.train(player,jewel,Attribute.Seduction);
			jewel.gainAffection(player,1);
			player.gainAffection(jewel,1);
		}
		else if(choice == "Games"){
			if(jewel.getAffection(player)>=8&&(!player.has(Trait.fearless)||Global.random(2)==1)){
				Global.gui().message("Today you've brought a wargame to appeal to Jewel's love of military history. You let her play the Allies, both because her father is a soldier and because the " +
						...
						"back. The sustained contact is taking a heavy toll on your troops, but you cycle the more damaged ones away from the front to minimize loses, while Jewel is losing units unneccesarily " +
						...
						"immediately plays it: Their Finest Hour. This is disasterous for you. She can attack with all her units this turn and they're all in firing range. She hits you across the whole front, " +
						"wiping out your damaged units and earning her just enough victory points to win the scenerio.<p>That was an extraordinary bit of luck for her. Those tactics would have been suicidal " +
				...
				if(!player.has(Trait.fearless)){
					...
					jewel.add(Trait.fearless);
				}
			}
			else{
				Global.gui().message("You were somewhat worried how Jewel would respond to the prospect of playing some potentially geeky games, but it seems your concerns were completely unfounded. "+
			...
			Global.gui().choose(this,"Leave");
			Daytime.train(player,jewel,Attribute.Cunning);
			jewel.gainAffection(player,1);
			player.gainAffection(jewel,1);
		}
		else if(choice == "Sparring"){
			if(jewel.getAffection(player)>=12&&(!player.has(Trait.dirtyfighter)||Global.random(2)==1)){
				Global.gui().message("Your martial arts training has finally reached the point where Jewel is willing to spar with you seriously. She's all business when it comes to sparring. There " +
						...
						"jukes out of the way of your counter. In theory, neither of you is trying to actually hurt the other, but if you miss a block she won't be able to pull the punch in time. " +
						...
						"takes a chance with a big roundhouse, you manage to block it and sweep her other leg, dropping her to the floor.<p>This is the first time you've managed a decisive takdown " +
						"against Jewel. You do your best to supress a grin as you reach down to help her back up. She catches you off guard by grabbing your wrists as her foot shoots up between your " +
						...
						"were vanquished.\"</i> She inexplicably begins a history lecture as if you weren't groaning in pain. <i>\"That's what saved them during the Second Punic War, when Hannibal outmanuevered " +
						...
						"<i>\"You haven't given up yet. Should I assume you want to keep fighting?\"</i> You hastily concede before your situation becomes any more painful. She breathes lustily in you ear and " +
						"fondles your penis. <i>\"Try to recover quickly. I need this in me as soon as possible.\"</i>");
				if(!player.has(Trait.dirtyfighter)){
					Global.gui().message("<p><b>You've learned the hard way that kicks can be dangerous, even when you're down.</b>");
					player.add(Trait.dirtyfighter);
					jewel.add(Trait.dirtyfighter);
				}
			}
			else{
				Global.gui().message("When you suggested a bit of sparring to Jewel, you weren't expecting anything like this. Despite your experience grappling and sex fighting each night, you don't " +
					...
					"minutes. You lose your boxers a few minutes later. To be fair, you've done well enough so far that Jewel is topless. You're still trying to get used to being naked outside during "+
			...
			Global.gui().choose(this,"Leave");
			Daytime.train(player,jewel,Attribute.Power);
			jewel.gainAffection(player,1);
			player.gainAffection(jewel,1);
		}
	...
	public void shop(Character npc, int budget) {
		npc.gainAffection(jewel,1);
		jewel.gainAffection(npc,1);
	}
