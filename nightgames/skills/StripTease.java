...
public class StripTease extends Skill {
	...
	public static boolean isUsable(Combat c, Character self, Character target) {
		return self.stripDifficulty(target) == 0 && self.canAct()&&c.getStance().mobile(self)&&!self.mostlyNude()&&!c.getStance().prone(self)&&c.getStance().front(self)&&!self.has(Trait.strapped);
	}
