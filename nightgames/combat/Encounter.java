...
public class Encounter implements Serializable{
	...
	public boolean spotCheck(){
		if(p1.eligible(p2)&&p2.eligible(p1)){
			...
			if(p1.state==State.masturbating){
				if(p1.human()){
					Global.gui().message(p2.name()+" catches you masturbating, but fortunately she's still not allowed to attack you, so she just watches you jerk off with " +
							"an amused grin.");
				}
				else if(p2.human()){
					Global.gui().message("You stumble onto "+p1.name()+" with her hand between her legs, masturbating. Since you just fought, you still can't touch her, so " +
							"you just watch the show until she orgasms.");
				}
			}
			else if(p2.state==State.masturbating){
				if(p2.human()){
					Global.gui().message(p1.name()+" catches you masturbating, but fortunately she's still not allowed to attack you, so she just watches you jerk off with " +
							"an amused grin.");
				}
				else if(p1.human()){
					Global.gui().message("You stumble onto "+p2.name()+" with her hand between her legs, masturbating. Since you just fought, you still can't touch her, so " +
							"you just watch the show until she orgasms.");
				}
			}
			else if(!p1.eligible(p2)&&p1.human()){
				Global.gui().message("You encounter "+p2.name()+", but you still haven't recovered from your last fight.");
			}
			else if (p1.human()){
				Global.gui().message("You find "+p2.name()+" still naked from your last encounter, but she's not fair game again until she replaces her clothes.");
			}
			location.endEncounter();
			return false;
		}
	...
	public void fightOrFlight(Character p,boolean fight){
		...
		if(checkin>=2){
			if(p1ff&&p2ff){
				...
				if(p2.check(Attribute.Speed, 10+p1.get(Attribute.Speed)+(p1.has(Trait.sprinter)?5:0)+(p2.has(Trait.sprinter)?-5:0))){
					if(p1.human()){
						Global.gui().message(p2.name()+" dashes away before you can move.");
					}
					p2.flee(location);
				}
				else{
					...
					if(p1.human()||p2.human()){
						if(p1.human()){
							Global.gui().message(p2.name()+" tries to run, but you stay right on her heels and catch her.");
						}
						else{
							Global.gui().message("You quickly try to escape, but "+p1.name()+" is quicker. She corners you and attacks.");
						}
						Global.gui().refresh();
						this.fight=Global.gui().beginCombat(p1, p2);
					}
				...
				if(p1.check(Attribute.Speed, 10+p2.get(Attribute.Speed)+(p1.has(Trait.sprinter)?-5:0)+(p2.has(Trait.sprinter)?5:0))){
					if(p2.human()){
						Global.gui().message(p1.name()+" dashes away before you can move.");
					}
					p1.flee(location);
				}
				else{
					...
					if(p1.human()||p2.human()){
						if(p2.human()){
							Global.gui().message(p1.name()+" tries to run, but you stay right on her heels and catch her.");
						}
						else{
							Global.gui().message("You quickly try to escape, but "+p2.name()+" is quicker. She corners you and attacks.");
						}
						this.fight=Global.gui().beginCombat(p1, p2);
					}
					else{
	...
	public void ambush(Character attacker,Character target){
		...
		if(p1.human()||p2.human()){
			this.fight=Global.gui().beginCombat(attacker, target,0);
			if(target.human()){
				Global.gui().message(attacker.name()+" catches you by surprise and attacks!");
			}
		}
		else{			
			//this.fight=new NullGUI().beginCombat(p1,p2);
			Global.gui().refresh();
			this.fight=new Combat(attacker,target,location,0);
		}
	}
	public void showerambush(Character attacker, Character target){
		...
		if(target.human()){
			if(location.id()==Movement.shower){
				Global.gui().message("You aren't in the shower long before you realize you're not alone. Before you can turn around, a soft hand grabs your exposed penis. "+attacker.name()+" has the drop on you.");
			}
			else if(location.id()==Movement.pool){
				Global.gui().message("The relaxing water causes you to lower your guard a bit, so you don't notice "+attacker.name()+" until she's standing over you. There's no chance to escape, you'll have to face her nude.");
			}
		}
		else if(attacker.human()){
			if(location.id()==Movement.shower){
				Global.gui().message("You stealthily walk up behind "+target.name()+", enjoying the view of her wet naked body. When you stroke her smooth butt, " +
		...
		if(p1.human()||p2.human()){
			Global.gui().refresh();
			this.fight=Global.gui().beginCombat(p1, p2,1);
		}
		else{			
			//this.fight=new NullGUI().beginCombat(p1,p2);
			this.fight=new Combat(p1,p2,location,0);
		}
	}
		...
		if(target.human()){
			if(location.id()==Movement.shower){
				Global.gui().message("The hot shower takes your fatigue away, but you can't seem to calm down. Your cock is almost painfully hard. You need to deal with this while " +
						...
						"spun around. "+attacker.name()+" has caught you at your most vulnerable and, based on her expression, may have been waiting for this moment. She kisses you and " +
			...
			if(location.id()==Movement.shower){
				Global.gui().message("You empty the bottle of aphrodisiac onto the shower floor, letting the heat from the shower turn it to steam. You watch "+target.name()+" and wait " +
	...
	public void caught(Character attacker, Character target){
		...
		if(target.human()){
			...
			Global.gui().message("You spot "+target.name()+" leaning against the wall with her hand working excitedly between her legs. She is mostly, but not completely successful at " +
	...
	public void spider(Character attacker, Character target){
		...
		if(attacker.human()){
			...
			Global.gui().message("You're trying to figure out a way to free yourself, when you see "+attacker.name()+" approach. You groan in resignation. There's no way you're " +
	...
	public void trap(Character opportunist, Character target, Trap trap){
		if(opportunist.human()){
			Global.gui().message("You leap out of cover and catch "+target.name()+" by surprise.");
		}
		else if(target.human()){
			Global.gui().message("Before you have a chance to recover, "+opportunist.name()+" pounces on you.");
		}
		Global.gui().refresh();
